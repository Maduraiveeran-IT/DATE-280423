    ...
    public partial class FrmSelectionTool : Form
    {
        ...
        public void Form1_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (Dv.Table.Columns[1].ColumnName.Contains("TNO") == true)
                    {
        ...
        private void Grid_Design(ref DataGridView DGV, DataTable Dt)
        {
            try
            {
                ...
                {
                    if (Dt.Columns[i].ColumnName.ToUpper().Contains("AMOUNT") || Dt.Columns[i].ColumnName.ToUpper().Contains("PRICE") || Dt.Columns[i].ColumnName.ToUpper().Contains("_PER") || Dt.Columns[i].ColumnName.ToUpper().Contains("CREDIT") || Dt.Columns[i].ColumnName.ToUpper().Contains("DEBIT"))
                    {
            ...
            try
            {
                if (TxtCriteria.Text.Trim() != String.Empty)
                {
                    ...
                    else if (Txt_No == 2)
                    {
                        ...
                        {
                            if (CmbFilter.Text.Contains("TNO") == true)
                            {
        ...
        private void BtnMinus_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cmblist.Items.RemoveAt(cmblist.SelectedIndex);
        ...
        private void BtnPlus_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (TxtCriteria.Visible == false && DtFrom.Visible == true && DtTo.Visible == true)
                {
                    Str = CmbFilter.Text + " >= #" + String.Format("{0:MM/dd/yyyy}", DtFrom.Value) + "# and " + CmbFilter.Text + " <= #" + String.Format("{0:MM/dd/yyyy}", DtTo.Value) + "#";
                    cmblist.Items.Add(Str);
                    cmblist.SelectedItem = Str;
                }
                else
                {
                    if (TxtCriteria.Text.Trim() != String.Empty)
                    {
                        if (CmbCondition.Text.ToUpper().Contains("PART"))
                        {
                            Str = CmbFilter.Text + "-->" + CmbCondition.Text + "-->'%" + TxtCriteria.Text + "%'";
                        }
                        else if (CmbCondition.Text.ToUpper().Contains("STARTS WITH"))
                        {
                            Str = CmbFilter.Text + "-->" + CmbCondition.Text + "-->'%" + TxtCriteria.Text + "'";
                        }
                        else if (CmbCondition.Text.ToUpper().Contains("NOT LIKE"))
                        {
                            Str = CmbFilter.Text + "-->" + CmbCondition.Text + "-->'%" + TxtCriteria.Text + "%'";
                        }
                        else
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                for (int i = 0; i <= cmblist.Items.Count - 1; i++)
                {
                    ActStr = cmblist.Items[i].ToString().Replace("-->", " ");
                    ActStr = ActStr.Replace("Part", "Like");
                    ActStr = ActStr.Replace("Starts With", "Like");
                    ActStr = ActStr.Replace("Equal To", "=");
                    ActStr = ActStr.Replace("Greater Than", ">");
                    ActStr = ActStr.Replace("Less Than", ">");
