...
namespace Accounts
{
    public partial class FrmNeedleMaster : Form, Entry 
    {
        ...
        public void Entry_Save()
        {
            try
            {
                ...
                if (MyParent._New)
                {
                    MyBase.Execute("Insert into VFit_Sample_Needle_Master (Name, Remarks, UserCode, Syscode, EntryAt) values ('" + TxtName.Text.Trim() + "', '" + TxtRemarks.Text.Trim() + "', " + MyParent.UserCode + ", " + MyParent.SysCode + ", getdate())");
                }
                else
                {
                    MyBase.Run("update VFit_Sample_Needle_Master Set Name = '" + TxtName.Text.Trim() + "', Entryat = GetDate(), Remarks = '" + TxtRemarks.Text.Trim() + "' Where RowID = " + Code);
                }
        ...
        public void Entry_Edit()
        {
            try
            {
                MyBase.Enable_Controls(this, true);
                MyBase.Clear(this);
                Dr = Tool.Selection_Tool(this, 200, 100, SelectionTool_Class.ViewType.NormalView, "Needle Master - Edit", "Select Name, Remarks, RowID from VFit_Sample_Needle_Master order by Name", "", 250, 200);
                Fill_Datas(Dr);
                TxtName.Focus();
            }
            catch (Exception ex)
        ...
        public void Entry_Delete()
        {
            try
            {
                ...
                MyBase.Clear(this);
                Dr = Tool.Selection_Tool(this, 200, 100, SelectionTool_Class.ViewType.NormalView, "Needle Master - Delete", "Select Name, Remarks, RowID from VFit_Sample_Needle_Master order by Name", "", 250, 200);
                Fill_Datas(Dr);
                if (Dr != null)
                {
        ...
        public void Entry_Delete_Confirm()
        {
            try
            {
                ...
                if (Code > 0)
                {
                    Sql = "Delete from VFit_Sample_Needle_Master Where RowID = " + Code;
                    MyBase.Execute(Sql);
        ...
        public void Entry_View()
        {
            try
            {
                MyBase.Enable_Controls(this, false);
                MyBase.Clear(this);
                Dr = Tool.Selection_Tool(this, 200, 100, SelectionTool_Class.ViewType.NormalView, "Needle Master - View", "Select Name, Remarks, RowID From VFit_Sample_Needle_Master order by Name", "", 250, 200);
                Fill_Datas(Dr);
            }
            catch (Exception ex)
            {
