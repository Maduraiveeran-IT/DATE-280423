        ...
        void Sample_Selection1()
        {
            try
            {
                if (TxtOrder.Text.ToString() != String.Empty)
                {
                    ...
                    {
                        Str3 = " Select Sample, Size, BOM, Production, Qty, OrderColorID, Cycle_Seconds, Cycle_Time, Assign_Qty, Bal_Qty, Prod, Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning('" + TxtOrder.Text + "', " + TxtTNeedle.Tag.ToString() + ")";
                    }
                    Dr = Tool.Selection_Tool_WOMDI(this, 500, 180, SelectionTool_Class.ViewType.NormalView, "Select Sample", Str3, String.Empty, 100, 80, 80, 80);
                    if (Dr != null)
                    {
