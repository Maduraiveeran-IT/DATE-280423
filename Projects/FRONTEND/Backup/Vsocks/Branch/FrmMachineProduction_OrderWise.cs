            ...
            try
            {
                if (e.KeyCode == Keys.Down && Grid.CurrentCell.ColumnIndex == Grid.Columns["Sample"].Index)
                {
                    ...
                    if (Grid["Order_No", Grid.CurrentCell.RowIndex].Value == "O")
                    {
                        ...
                        if (LblUnit == 1)
                        {
                            Dr = Tool.Selection_Tool_Except_New_WOMDI("Sample", this, 500, 180, ref Dt, SelectionTool_Class.ViewType.NormalView, "Select Sample", "Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time From Get_Order_Sample_For_Produciton_planning_OLD( '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + Grid["Needle_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Where B.Order_No = '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "' And B.SupplierID = 71", String.Empty, 100, 80, 80, 80);
                        }
                        else if (LblUnit == 2)
                        {
                            Dr = Tool.Selection_Tool_Except_New_WOMDI("Sample", this, 500, 180, ref Dt, SelectionTool_Class.ViewType.NormalView, "Select Sample", "Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time From Get_Order_Sample_For_Produciton_planning_OLD( '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + Grid["Needle_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Where B.Order_No = '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "' And B.SupplierID = 72", String.Empty, 100, 80, 80, 80);
                        }
                        else
                        {
                            Dr = Tool.Selection_Tool_Except_New_WOMDI("Sample", this, 500, 180, ref Dt, SelectionTool_Class.ViewType.NormalView, "Select Sample", "Select Sample, Size, BOM, Production, Qty, OrderColorID, Cycle_Seconds, Cycle_Time From Get_Order_Sample_For_Produciton_planning_OLD( '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + Grid["Needle_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ")", String.Empty, 100, 80, 80, 80);
                        }
                    }
                    else
                    {
                        ...
                        {
                            Dr = Tool.Selection_Tool_WOMDI(this, 500, 180, SelectionTool_Class.ViewType.NormalView, "Select Sample", "Select A.Sample, A.Size, A.BOM, A.Production, A.Balance, A.Allocated_Machines, A.Allocated_Qty, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time From Get_Order_Sample_For_Produciton_planning_Allocation_Based( " + LblYear.Text + ", " + LblWeek.Text + ", '" + LblDate.Text + "', " + LblShift.Text + ", '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + Grid["Needle_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ")A ", String.Empty, 100, 60, 60, 60, 60, 60, 60, 60);
                        }
                    }
                    if (Dr != null)
                    {
