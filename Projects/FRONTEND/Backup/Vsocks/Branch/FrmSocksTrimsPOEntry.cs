...
namespace Accounts
{
    public partial class FrmSocksTrimsPOEntry : Form, Entry
    {
        ...
        public void Entry_Save()
        {
            ...
            try
            {
                ...
                if (MyParent._New)
                {
                    Queries[Array_Index++] = "Insert Into Socks_Trims_PO_Master (PoNo, PODate, Supplier_Code, Required_Date, PO_Method, Buyer_Code, Remarks) Values ('" + TxtPONO.Text + "', '" + String.Format("{0:dd-MMM-yyyy}", DtpDate.Value) + "', " + TxtSupplier.Tag.ToString() + ", '" + String.Format("{0:dd-MMM-yyyy}", DtpReqDate.Value) + "', " + CmbBasedOn.SelectedIndex + ", " + TxtBuyer.Tag.ToString() + ", '" + TxtRemarks.Text + "'); Select Scope_Identity ()";
                }
                else
                {
                    Queries[Array_Index++] = "update Socks_Trims_PO_Master Set Supplier_Code = " + TxtSupplier.Tag.ToString() + ", Remarks = '" + TxtRemarks.Text + "', Required_Date = '" + String.Format ("{0:dd-MMM-yyyy}", DtpReqDate.Value) + "' Where RowID = " + Code;
                ...
                else
                {
                    ...
                    for (int i = 0; i <= Dt_Item.Rows.Count - 1; i++)
                    {
                        for (int j = 0; j <= Dt_Item_OCN[i].Rows.Count - 1; j++)
                        {
                            if (MyParent._New)
                            {
                                if (Dt_Item_OCN[i].Rows[j]["PO_Qty"].ToString() != "0.000")
                                {
                         ...
                         //  Queries[Array_Index++] = "UPdate Socks_Trims_BOM_Status Set PO_Qty = PO_Qty + ((Case When ((BOM + SPec_Req) - PO_Qty) < " + Dt_Item_OCN[i].Rows[j]["PO_Qty"].ToString() + " then ((BOM + SPec_Req) - PO_Qty) else " + Dt_Item_OCN[i].Rows[j]["PO_Qty"].ToString() + " End))  Where Order_ID = " + Dt_Item_OCN[i].Rows[j]["Order_ID"].ToString() + " and Item_ID = " + Dt_Item.Rows[i]["Item_ID"].ToString() + " and Color_ID = " + Dt_Item.Rows[i]["Color_ID"].ToString() + " and Size_ID = " + Dt_Item.Rows[i]["Size_ID"].ToString();
        ...
        public void Entry_View()
        {
            try
            {
                MyBase.Enable_Controls(this, false);
                MyBase.Clear(this);
                Dr = Tool.Selection_Tool_Resize(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select PO Details - View", "Select S1.PONo, L1.Ledger_Name Supplier, S1.PoDate, S3.Order_No, i1.item, c1.color, s4.size, s2.Order_Qty, S1.RowID, S1.PO_Method, S1.Supplier_Code, S1.Buyer_Code, L2.Ledger_Name Buyer, S1.Remarks From Socks_Trims_PO_Master S1 Inner join Socks_Trims_PO_Details S2 ON S1.RowID = s2.Master_ID Inner join Socks_Order_Master S3 on S2.Order_ID = S3.RowID inner join item I1 on S2.Item_id = I1.itemid inner join color C1 on s2.Color_id = c1.colorid inner join size S4 on s2.Size_ID = S4.sizeid left Join Supplier_All_Fn() L1 On L1.LEdgeR_Code = S1.Supplier_Code left Join Accounts.Dbo.Ledger_Master L2 on S1.buyer_Code = L2.Ledger_Code and L2.COMPANY_CODE = " + MyParent.CompCode + " and L2.YEAR_CODE = '" + MyParent.YearCode + "' Where  S1.RowID != 168", String.Empty, 120, 250, 100, 120, 120, 120, 100, 100);
                if (Dr != null)
                {
                    Fill_Data(Dr);
                }
        ...
        public void Entry_Print()
        {
            try
            {
                ...
                Str1 = " Select Top 100000000 ROW_NUMBER()Over(Order By I1.Item + ' - ' + C1.color + ' - ' + S4.Size)SlNo, I1.Item + ' - ' + C1.color + ' - ' + S4.Size Item_Color_Size, S2.Order_Qty, S2.Rate, S2.Amount, S1.Remarks From Socks_Trims_PO_Master S1 Inner join Socks_Trims_PO_Details S2 ON S1.RowID = S2.Master_ID Inner join Socks_Order_Master S3 on S2.Order_ID = S3.RowID Inner join Item I1 on S2.Item_id = I1.itemid Inner join color C1 on s2.Color_id = c1.colorid Inner join size S4 on s2.Size_ID = S4.sizeid Where S2.Order_Qty > 0 and S1.RowID = " + Code + " Order By I1.Item + ' - ' + C1.color + ' - ' + S4.Size ";
                ...
                Str3 = " Select Distinct S3.Order_No From Socks_Trims_PO_Master S1 Inner join Socks_Trims_PO_Details S2 ON S1.RowID = S2.Master_ID Inner join Socks_Order_Master S3 on S2.Order_ID = S3.RowID Where S1.RowID = " + Code + " and S2.Order_Qty > 0 ";
                ...
                else if (Res == DialogResult.No)
                {
                    ...
                    MyParent.CReport_Normal_PDF(ref ObjRpt, "Accessories Purchase Order..!", "C:\\Vaahrep\\GainupPO.Pdf", false);
        ...
        private void FrmSocksYarnPOEntry_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    if (this.ActiveControl.Name == "TxtBuyer")
                    {
                         Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Buyer", " Select LedgeR_Name Buyer, LedgeR_Code Code From Buyer_All_Fn() Where LEdgeR_code != 8102", String.Empty, 250);
                        if (Dr != null)
                        {
                            TxtBuyer.Text = Dr["Buyer"].ToString();
