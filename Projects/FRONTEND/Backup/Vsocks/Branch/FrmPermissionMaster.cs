...
namespace Accounts
{
    public partial class FrmPermissionMaster : Form, Entry 
    {
        Control_Modules MyBase = new Control_Modules();
        MDIMain MyParent;
        int New, Edit, Delete, Preview, Print, View;
        SelectionTool_Class Tool = new SelectionTool_Class();
        int User_Level_Code = 0;
        DataRow Dr;
        String Code;

        public FrmPermissionMaster()
        {
            InitializeComponent();
        }
        ...
        void NewField()
        {
            try
            {
                MyBase.Add_NewField("Socks_Permission_Master", "New_empCode", "int");
                MyBase.Add_NewField("Socks_Permission_Master", "New_SysCode", "int");
                MyBase.Add_NewField("Socks_Permission_Master", "New_DateTime", "DateTime");
                MyBase.Add_NewField("Socks_Permission_Master", "Alter_empCode", "int");
                MyBase.Add_NewField("Socks_Permission_Master", "Alter_SysCode", "int");
                MyBase.Add_NewField("Socks_Permission_Master", "Alter_DateTime", "DateTime");
                MyBase.Add_NewField("Socks_Permission_Master", "Company_Code", "int");
                MyBase.Add_NewField("Socks_Permission_Master", "Year_Code", "Varchar(10)");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        void TableCreation()
        {
            try
            {
                if (MyBase.Check_Table("Socks_Permission_Master"))
                {
                    if (MyBase.Check_TableField("Socks_Permission_Master", "SA_PR"))
                    {
                        MyBase.Execute("Drop table Socks_Permission_Master");
                    }
                }
                if (MyBase.Check_Table ("Socks_Permission_Master") == false)
                {
                    MyBase.Execute ("Create table Socks_Permission_Master (Menu_Code int, User_Code int, Status varchar(4), Opt_New int, Opt_Edit int, Opt_Delete int, Opt_View int, Opt_PrintPreview int, Opt_Print int)");
                }
                NewField();
            }
            catch (Exception ex)
        ...
        void Load_tree()
        {
            ...
            try
            {
                ...
                MyBase.Load_Data("Select Menu_Code Code, Menu_Name Menu From Menu_Master Where Under = 'Main' and menu_Code in (Select Menu_Code from Socks_Permission_Master_User_Level_Fixed where User_Level_Code = " + User_Level_Code + ") order by Menu_Code", ref Dt);
                for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                {
                    treeView1.Nodes.Add(Dt.Rows[i]["Code"].ToString(), Dt.Rows[i]["Menu"].ToString().Replace("&",""));
                    MyBase.Load_Data("Select Menu_Code Code, Menu_Name Menu, Under From Menu_Master Where under = '" + Dt.Rows[i]["Code"].ToString() + "' and menu_Code in (Select Menu_Code from Socks_Permission_Master_User_Level_Fixed where User_Level_Code = " + User_Level_Code + ") order by Menu_Code", ref Dt1);
                    for (int j = 0; j <= Dt1.Rows.Count - 1; j++)
                    {
                        ...
                        MyBase.Load_Data("Select Menu_Code Code, Menu_Name Menu, Under From Menu_Master Where under = '" + Dt1.Rows[j]["Code"].ToString() + "' and menu_Code in (Select Menu_Code from Socks_Permission_Master_User_Level_Fixed where User_Level_Code = " + User_Level_Code + ") order by Menu_Code", ref Dt2);
        ...
        private void TxtBankName_KeyDown_1(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                {
                    Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select User ", "Select u1.User_Name, u2.User_Level_Fixed, u1.USer_Code Code, u1.User_Level_Code from Fixed_User_Master u1 left join User_Level_Fixed u2 on u1.User_Level_Code = u2.User_Level_Code order by user_name", "", 250, 150);
                    if (Dr != null)
                    {
                        TxtBankName.Text = Dr["User_Name"].ToString();
                        TxtBankName.Tag = Dr["Code"].ToString();
                        User_Level_Code = Convert.ToInt32(Dr["User_Level_Code"]);
                        Load_tree();
                    }
                    else
                    {
        ...
        void Enable_Menu(String Menu_Code)
        {
            try
            {
                MyBase.Execute("Delete from Socks_Permission_Master where Menu_Code = " + Menu_Code + " and user_Code = " + TxtBankName.Tag.ToString());
                MyBase.Execute("insert into Socks_Permission_Master values (" + Menu_Code + "," + TxtBankName.Tag.ToString() + ",'True', " + New + ", " + Edit + ", " + Delete + ", " + View + ", " + Preview + ", " + Print + ", " + MyParent.UserCode + ", " + MyParent.SysCode + ", " + MyParent.Today + "," + MyParent.UserCode + ", " + MyParent.SysCode + ", " + MyParent.Today + "," + MyParent.CompCode + ", '" + MyParent.YearCode + "')");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private void ButSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Rights();
                MyBase.Execute("Delete from Socks_Permission_Master where user_Code = " + TxtBankName.Tag.ToString());
        ...
        private void ButReset_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Sure to Reset Permission for this USER ...!", "Reset ?", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    MyBase.Execute("Delete from Socks_Permission_Master where user_Code = " + TxtBankName.Tag.ToString());
        ...
        private void TxtCopyFrom_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Down)
                {
                    Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Copy From", "Select User_Name, User_Code from Fixed_User_Master where user_Code in (Select USer_Code from Socks_Permission_Master) order by user_Name", "", 250);
                    if (Dr != null)
                    {
