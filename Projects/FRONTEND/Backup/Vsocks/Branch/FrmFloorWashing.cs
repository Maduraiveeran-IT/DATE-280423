...
namespace Accounts
{
    public partial class FrmFloorWashing : Form, Entry 
    {
        ...
        public FrmFloorWashing()
        {
            InitializeComponent();
        }

        private void FrmFloorWashing_Load(object sender, EventArgs e)
        {
            try
            {
                MyParent = (MDIMain)this.MdiParent;
                Buffer_Table = "Wash_" + Environment.MachineName.Replace("-", "") + "_" + MyParent.UserCode.ToString();
                MyBase.Clear(this);
            }
            catch (Exception ex)
            {
        ...
        private void FrmFloorWashing_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void FrmFloorWashing_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
        ...
        public void Entry_Edit()
        {
            try
            {
                ...
                Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Washing - Edit", "Select F1.EntryNo, F1.ENtryDate, S1.Shiftcode2 Shift, F1.Timing, F1.ShiftCode, F1.Remarks, F1.RowID From Floor_Washing_Master F1 Left Join Socks_Shift () S1 on F1.ShiftCode = S1.Shiftcode Where F1.ENtryDate >='14-nov-2015' and F1.ENtryDate >= Dateadd (D, " + MyParent.User_Datelock + ", Cast(GetDate() as Date))", String.Empty, 80, 90, 70);
                if (Dr != null)
                {
                    Fill_Datas(Dr);
                }
        ...
        decimal Fill_BOM_Check(String OrderNo, String Sample, String Size, decimal Linked)
        {
            try
            {
                Int64 Prod = 0;
                ...
                decimal Linked1 = 0;
                DataTable Tdt = new DataTable();
                MyBase.Load_Data("Select S1.Bom_Qty Bom, cast(Isnull(F1.LinkQty, 0) as numeric(25,2)) Linked, cast((isnull(F1.LinkQty,0) - Isnull(K1.Washed,0)) as numeric(25,2)) Balance_Washing From Socks_Bom() S1 Left Join Floor_Stock F1 on S1.Order_No = F1.Order_No and S1.OrderColorId = F1.OrderColorID and S1.sizeid = F1.SizeID and S1.Bom_Qty = F1.BOMQty Left Join Washing_Production_All () k1 on S1.Order_No = K1.OrderNo and S1.OrderColorId = K1.OrderColorID and S1.sizeid = K1.SizeID Where S1.Order_No = '" + OrderNo + "' And S1.color = '" + Sample + "' and S1.Size = '" + Size + "'", ref Tdt);
                if (Tdt.Rows.Count > 0)
                {
                    Linked1 = Convert.ToDecimal(Tdt.Rows[0]["Linked"].ToString());
                    Bal = Linked1;
        ...
        public void Entry_Save()
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                {
                    ...
                    if (Fill_BOM_Check(Grid["Order_No", i].Value.ToString(), Grid["Sample", i].Value.ToString(), Grid["Size", i].Value.ToString(),Convert.ToDecimal(Grid["Linked_Qty", i].Value.ToString())) < 0)
                    {
                ...
                TxtNo.Text = MyBase.MaxOnlyComp("Floor_Washing_Master", "EntryNo", String.Empty, MyParent.YearCode, MyParent.CompCode).ToString();

                if (MyParent._New)
                {
                    Queries[Array_Index++] = "Insert into Floor_Washing_master (EntryNo, EntryDate, ShiftCode, Timing, Company_Code, EntryTime, EntrySystem, Remarks) Values (" + TxtNo.Text + ",'" + String.Format("{0:dd-MMM-yyyy}", DtpDate1.Value) + "','" + TxtShift.Tag.ToString() + "','" + TxtTiming.Text.ToString() + "'," + MyParent.CompCode + ",getdate(),Host_name(), '" + TxtRemarks.Text + "') ; Select Scope_Identity()";
                }
                else
                {
                    Queries[Array_Index++] = "Update Floor_Washing_Master Set ShiftCode = " + TxtShift.Tag.ToString() + ", Remarks = '" + TxtRemarks.Text + "' where RowID = " + Code;
                    ...
                    Queries[Array_Index++] = "Update F1 Set F1.WashQty = F1.WashQty - Isnull(F2.Production, 0) From Floor_Stock F1 Inner join (Select Order_No, OrderColorID, SizeID, Sum(Production) Production From Floor_Washing_Details Where MasterID = " + Code + " Group By Order_No, OrderColorID, SizeID) F2 on F1.Order_No = F2.Order_No and F1.OrderColorID = F2.OrderColorID and F1.SizeID = F2.SizeID ";
                    Queries[Array_Index++] = "Delete From Floor_Washing_Details where MAsterID = " + Code;
                }
                ...
                for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                {
                    ...
                    {
                        Queries[Array_Index++] = "Insert Into Floor_Washing_Details (MasterID, MachineID, Tag_No, Order_No, OrderColorID, ItemID, SizeID, BOMQty, production, Emplno_Operator) Values (@@IDENTITY, '" + Grid["Machine", i].Value.ToString() + "', '" + Grid["Tag_No", i].Value.ToString() + "', '" + Grid["Order_No", i].Value.ToString() + "', " + Grid["OrderColorID", i].Value.ToString() + ", " + Grid["ItemID", i].Value + ", " + Grid["SizeID", i].Value.ToString() + ", " + Grid["Bom", i].Value + ", " + Grid["Production", i].Value + ", " + Grid["Emplno_Operator", i].Value + ")";
                    }
                    else
                    {
                        Queries[Array_Index++] = "Insert Into Floor_Washing_Details (MasterID, MachineID, Tag_No, Order_No, OrderColorID, ItemID, SizeID, BOMQty, production, Emplno_Operator) Values (" + Code + ", '" + Grid["Machine", i].Value.ToString() + "', '" + Grid["Tag_No", i].Value.ToString() + "', '" + Grid["Order_No", i].Value.ToString() + "', " + Grid["OrderColorID", i].Value.ToString() + ", " + Grid["ItemID", i].Value + ", " + Grid["SizeID", i].Value.ToString() + ", " + Grid["Bom", i].Value + ", " + Grid["Production", i].Value + ", " + Grid["Emplno_Operator", i].Value + ")";
                    }

                    Queries[Array_Index++] = " update Floor_Stock set Act_WashQty = Act_WashQty + " + Grid["Production", i].Value + " where Order_No = '" + Grid["Order_No", i].Value.ToString() + "' and OrderColorID = " + Grid["OrderColorID", i].Value + " and SizeID = '" + Grid["SizeID", i].Value.ToString() + "'";
                    Queries[Array_Index++] = " update Floor_Stock set WashQty = WashQty + " + Grid["Production", i].Value + " where Order_No = '" + Grid["Order_No", i].Value.ToString() + "' and OrderColorID = " + Grid["OrderColorID", i].Value + " and SizeID = '" + Grid["SizeID", i].Value.ToString() + "'";
                }
        ...
        public void Entry_Delete()
        {
            try
            {
                ...
                Set_Min_Max_Date(true);
                Buffer_Update = false;
                Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Washing - Delete", "Select F1.EntryNo, F1.ENtryDate, S1.Shiftcode2 Shift, F1.Timing, F1.ShiftCode, F1.Remarks, F1.RowID From Floor_Washing_Master F1 Left Join Socks_Shift () S1 on F1.ShiftCode = S1.Shiftcode where F1.ENtryDate >='14-nov-2015' ", String.Empty, 80, 90, 70);
                if (Dr != null)
                {
                    Fill_Datas(Dr);
        ...
        public void Entry_Delete_Confirm()
        {
            try
            {
                if (Code > 0)
                {
                    MyBase.Run("Update F1 Set F1.WashQty = F1.WashQty - Isnull(F2.Production, 0), F1.Act_WashQty = F1.Act_WashQty - Isnull(F2.Production, 0) From Floor_Stock F1 Inner join (Select Order_No, OrderColorID, SizeID, Sum(Production) Production From Floor_Washing_Details Where MasterID = " + Code + " Group By Order_No, OrderColorID, SizeID) F2 on F1.Order_No = F2.Order_No and F1.OrderColorID = F2.OrderColorID and F1.SizeID = F2.SizeID", "Delete From Floor_Washing_Details where MasterID = " + Code, "Delete From Floor_Washing_Master where RowID = " + Code);
        ...
        public void Entry_View()
        {
            try
            {
                ...
                Buffer_Update = false;
                Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Washing - View", "Select F1.EntryNo, F1.EntryDate, S1.Shiftcode2 Shift, F1.Timing, F1.ShiftCode, F1.Remarks, F1.RowID From Floor_Washing_Master F1 Left Join Socks_Shift () S1 on F1.ShiftCode = S1.Shiftcode", String.Empty, 80, 90, 70);
                if (Dr != null)
                {
                    Fill_Datas(Dr);
                }
        ...
        void Grid_Data(Boolean Buffer)
        {
            ...
            try
            {
                if (Buffer)
                {
                    Str = "Select 0 as Slno, F1.Tag_No, F1.Order_No, F1.OrderColorID, Sample, F1.ItemID, Item, F1.SizeID, S1.Size, F1.BOMQty Bom, Cast(0 as numeric(25,2)) Linked_Qty, F1.Production,  F1.Machine, F1.Emplno_OPerator, E1.Name OPerator, '-' T From " + Buffer_Table + " F1 Left Join size S1 on F1.SizeID = S1.sizeid Left Join Vaahini_ERP_Gainup.Dbo.Employeemas E1 on F1.Emplno_Operator = E1.Emplno Order By F1.Slno";
                }
                else
                {
                    if (MyParent._New)
                    {
                        Str = "Select 0 as Slno, F1.Tag_No, F1.Order_No, F1.OrderColorID, Cast('' As Varchar (15)) Sample, F1.ItemID, Cast('' As Varchar (15)) Item, F1.SizeID, Cast('' As Varchar (15)) Size, F1.BOMQty Bom, Cast(0 as numeric(25,2)) Linked_Qty, F1.Production,  F1.MachineID Machine, F1.Emplno_OPerator, E1.Name OPerator, '-' T From Floor_Washing_Details F1 Left Join size S1 on F1.SizeID = S1.sizeid Left Join Vaahini_ERP_Gainup.Dbo.Employeemas E1 on F1.Emplno_Operator = E1.Emplno Where 1 = 2";
                    }
                    else
                    {
                        Str = "Select 0 as Slno, F1.Tag_No, F1.Order_No, F1.OrderColorID, C1.color Sample, F1.ItemID, C1.item Item, F1.SizeID, S1.Size, F1.BOMQty Bom, F2.LinkQty Linked_Qty, F1.Production,  F1.MachineID Machine, F1.Emplno_OPerator, E1.Name OPerator, '-' T From Floor_Washing_Details F1 Left Join Socks_Bom() C1 On F1.OrderColorID = C1.OrderColorId And F1.Order_No = C1.Order_No Left Join Floor_Stock F2 On F1.Order_No = F2.Order_No and F1.OrderColorID = F2.OrderColorID and F1.SizeID = F2.SizeID Left Join size S1 on F1.SizeID = S1.sizeid Left Join Vaahini_ERP_Gainup.Dbo.Employeemas E1 on F1.Emplno_Operator = E1.Emplno Where F1.MasterID = " + Code + " Order By F1.RowID";
                    }
                }
                ...
                MyBase.Grid_Width(ref Grid, 40, 100, 120, 120, 100, 100, 100, 100, 100, 150, 200);
        ...
        void Txt_GotFocus(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (Grid.CurrentCell.ColumnIndex == Grid.Columns["Machine"].Index && Grid.CurrentCell.Value.ToString() == String.Empty)
                {
                    if (Grid.CurrentCell.RowIndex > 0)
                    {
                        Grid["Machine", Grid.CurrentCell.RowIndex].Value = Grid["Machine", Grid.CurrentCell.RowIndex - 1].Value.ToString();
                ...
                else if (Grid.CurrentCell.ColumnIndex == Grid.Columns["Tag_No"].Index && Grid.CurrentCell.Value.ToString() == String.Empty)
                {
                    if (Grid.CurrentCell.RowIndex > 0)
                    {
                        ...
                        Grid["Linked_Qty", Grid.CurrentCell.RowIndex].Value = Grid["Linked_Qty", Grid.CurrentCell.RowIndex - 1].Value.ToString();
        ...
        void Total_Prod_Qty()
        {
            ...
            {
                Grid.Refresh();
                TxtTotal.Text = String.Format("{0:0.0}", Convert.ToDouble(MyBase.Sum(ref Grid, "Production", "Tag_No", "Sample", "Operator")));
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        void Machine_Selection()
        {
            try
            {
                Dr = Tool.Selection_Tool(this, 150, 150, SelectionTool_Class.ViewType.NormalView, "Select Machine", "Select Machine From Washing_Mc_NO ()", String.Empty, 200);
                if (Dr != null)
                {
        ...
        void OrderNo_Selection()
        {
            try
            {
                Dr = Tool.Selection_Tool(this, 150, 150, SelectionTool_Class.ViewType.NormalView, "Select Tag No", "select Tag_No, Order_No, Item, Sample, Size, BOMQty BOM, Linked_Qty, ItemID, OrderColorID, SizeID from Floor_Washing_Input_New() Where Linked_Qty > 0", String.Empty, 100, 120, 100, 100, 100, 100, 100);
                if (Dr != null)
                {
                    ...
                    Grid["Linked_Qty", Grid.CurrentCell.RowIndex].Value = Dr["Linked_Qty"].ToString();
                    Grid["Production", Grid.CurrentCell.RowIndex].Value = Dr["Linked_Qty"].ToString();
        ...
        void Operator_Selection()
        {
            try
            {
                //Dr = Tool.Selection_Tool(this, 150, 150, SelectionTool_Class.ViewType.NormalView, "Select Operator", "Select Name, Tno, Emplno From Socks_Employee_Present_Detail ('" + String.Format("{0:dd-MMM-yyyy}", DtpDate1.Value) + "') Where DeptName = 'Washing' and Tno Not Like '%Z'", String.Empty, 250, 80);
                Dr = Tool.Selection_Tool(this, 150, 150, SelectionTool_Class.ViewType.NormalView, "Select Operator", "select Name, Tno, Emplno from VAAHINI_ERP_GAINUP.dbo.Employeemas where Emplno in (176, 7966, 11663, 11747, 12566) And Tno Not like '%Z'", String.Empty, 250, 80);
                if (Dr != null)
                {
        ...
        void Fill_BOM(String OrderNo, String Sample, String Size)
        {
            ...
            {
                DataTable Tdt = new DataTable();
                MyBase.Load_Data("Select F1.BomQty Bom, cast(isnull(F1.LinkQty,0) as numeric(25,2)) Linked, cast(Isnull(L1.Washed, 0) as numeric(25,2)) Washed, (isnull(F1.LinkQty,0) - Isnull(L1.Washed, 0)) Balance_Washing From Floor_Stock F1 left join Socks_Bom () S1 on F1.Order_No = S1.Order_No and F1.OrderColorID = S1.OrderColorId and F1.SizeID = S1.sizeid Left Join Washing_Production_All () L1 on F1.Order_No = L1.OrderNo and F1.OrderColorId = L1.OrderColorID and F1.sizeid = L1.SizeID Where F1.Order_No = '" + OrderNo + "' And S1.color = '" + Sample + "' and S1.Size = '" + Size + "'", ref Tdt);
                if (Tdt.Rows.Count > 0)
                {
                    LblLinked.Text = "Linked: " + Tdt.Rows[0]["Linked"].ToString();
                    LblPre_Prod.Text = "PROD: " + Tdt.Rows[0]["Washed"].ToString();
                    LblBal.Text = "BAL: " + Tdt.Rows[0]["Balance_Washing"].ToString();
                ...
                {
                    Tdt = new DataTable();
                    MyBase.Load_Data("Select Isnull(Sum(Production), 0) Production From Floor_Washing_Details Where Order_No = '" + OrderNo + "' And OrderColorID = .Dbo.Get_OrdercolorID ('" + OrderNo + "', '" + Sample + "') and SizeID = Dbo.Get_OrderSizeID ('" + OrderNo + "', '" + Size + "') and MasterID = " + Code, ref Tdt);
        ...
        
        {
            ...
            try
            {
                if (Buffer_Update)
                {
                    ...
                    {
                        MyBase.Execute("Select Cast(0 as int) Slno, Tag_No, Order_No, OrderColorID,cast('' as varchar(20)) Sample, ItemID,cast('' as varchar(20)) Item, SizeID,cast('' as varchar(20)) Size, BomQty, Cast(0 as numeric(25,2)) Linked_Qty, Production, MachineID Machine, Emplno_Operator,cast('' as varchar(20)) Operator into " + Buffer_Table + " From Floor_Washing_Details Where 1 = 2");
                    }
                    ...
                    for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                    {
                        Queries[Array_Index++] = "Insert Into " + Buffer_Table + " (Slno, Tag_No, Order_No, OrderColorID, Sample, ItemID, Item, SizeID, Size, BOMQty, Linked_Qty, production, Machine, Emplno_Operator, Operator) Values (" + Grid["Slno", i].Value.ToString() + ", '" + Grid["Tag_No", i].Value.ToString() + "','" + Grid["Order_No", i].Value.ToString() + "', " + Grid["OrderColorID", i].Value.ToString() + ", '" + Grid["Sample", i].Value.ToString() + "', " + Grid["ItemID", i].Value.ToString() + ", '" + Grid["Item", i].Value.ToString() + "', " + Grid["SizeID", i].Value + ", '" + Grid["Size", i].Value.ToString() + "', " + Grid["Bom", i].Value + ", " + Grid["Linked_Qty", i].Value + ", " + Grid["Production", i].Value + ",  '" + Grid["Machine", i].Value.ToString() + "', " + Grid["Emplno_Operator", i].Value + ", '" + Grid["Operator", i].Value.ToString() + "')";
                    }
