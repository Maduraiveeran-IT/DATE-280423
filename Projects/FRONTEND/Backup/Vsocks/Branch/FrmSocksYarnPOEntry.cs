    ...
    public partial class FrmSocksYarnPOEntry : Form, Entry
    {
        ...
        public void Entry_View()
        {
            try
            {
                MyBase.Enable_Controls(this, false);
                MyBase.Clear(this);
                Dr = Tool.Selection_Tool_Resize(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select PO Details - View", "Select S1.PONo, L1.Ledger_Name Supplier, S1.PoDate, S3.Order_No, i1.item, c1.color, s4.size, s2.Order_Qty, S1.RowID, S1.PO_Method, S1.Supplier_Code, S1.Buyer_Code, L2.Ledger_Name Buyer From Socks_Yarn_PO_Master S1 Inner join Socks_Yarn_PO_Details S2 ON S1.RowID = s2.Master_ID Inner join Socks_Order_Master S3 on S2.Order_ID = S3.RowID inner join item I1 on S2.Item_id = I1.itemid inner join color C1 on s2.Color_id = c1.colorid inner join size S4 on s2.Size_ID = S4.sizeid left Join Supplier_All_Fn() L1 On L1.LEdgeR_Code = S1.Supplier_Code   left Join Accounts.Dbo.Ledger_Master L2 on S1.buyer_Code = L2.Ledger_Code and L2.COMPANY_CODE = " + MyParent.CompCode + " and L2.YEAR_CODE = '" + MyParent.YearCode + "'  and S1.RowID != 135 ", String.Empty, 120, 250, 100, 120, 120, 120, 100, 100);
                if (Dr != null)
                {
                    Fill_Data(Dr);
                }
            ...
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    else if (this.ActiveControl.Name == "TxtSupplier")
                    {
                        if (Grid.Rows.Count <=1 || MyParent.UserCode ==1)
                        {
                            Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Supplier", "Select LedgeR_Name Supplier, LedgeR_Code Code From Supplier_All_Fn() Where LEdgeR_code != 793 ", String.Empty, 250);
                            if (Dr != null)
                            {
                                TxtSupplier.Text = Dr["Supplier"].ToString();
        ...
        void Load_Item()
        {
            try
            {
                ...
                else
                {
                    ...
                    Grid_Item.DataSource = MyBase.Load_Data("Select 0 as Sl, S2.Size + ' ' + I1.Item + ' ' + C1.Color + ' @ ' + Cast(S1.Rate as Varchar (15)) Description, S1.Item_ID, I1.Item ITEM, S1.Color_ID, C1.Color COLOR, S1.Size_ID, S2.size SIZE, Sum(S1.BOM_Qty) BOM_Qty, (Sum(S1.Ordered_Qty) + Sum(S3.Order_Qty)) Ordered_Qty, (Sum(S1.BOM_Qty) - (Sum(S1.Ordered_Qty) + Sum(S3.Order_Qty)) +Sum(S3.Order_Qty)) Bal_Qty, Sum(S3.Order_Qty) PO_Qty, S1.RATE ARATE, S3.RATE, SUM(S3.Amount) AMOUNT From SocksPO_Pending_Items_All() S1 Inner Join Socks_Yarn_PO_Details S3 on S1.Order_ID = S3.Order_ID and S1.Item_ID = S3.Item_id and S1.Color_ID = S3.Color_id and S1.Size_ID = S3.Size_ID  Inner Join item I1 on S1.Item_id = I1.itemid Inner Join color C1 on s1.Color_id = c1.colorid Inner Join size S2 on s1.Size_ID = S2.sizeid Where Master_ID = " + Code + " Group By S1.Item_ID, I1.Item, S1.Color_ID, C1.Color, S1.Size_ID, S2.size, S1.Rate, S3.Rate", ref Dt_Item);
                }
        ...
        void Load_Dt_Item_OCN()
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= TempDt.Rows.Count - 1; i++)
                {
                    ...
                    MyBase.Load_Data("Select 0 as Sl, S1.Order_ID, S2.Order_No ORDER_NO, L1.Ledger_Name BUYER, S1.BOM_Qty, (S1.Ordered_Qty + S3.Order_Qty) Ordered_Qty, (S1.BOM_Qty - (S1.Ordered_Qty + S3.Order_Qty) + S3.OrdeR_Qty) Bal_Qty, S3.Order_Qty PO_Qty, '' T From SocksPO_Pending_Items_All() S1 Inner Join Socks_Order_Master S2 on S1.Order_ID = S2.RowID Inner Join Socks_Yarn_PO_Details S3 On S1.Order_ID = S3.Order_ID and  S1.Item_ID = S3.Item_id and S1.Color_ID = S3.Color_id and S1.Size_ID = S3.Size_ID Left Join Accounts.Dbo.Ledger_Master L1 on S2.Party_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Where S3.Master_ID = " + Code + " and S1.Item_ID = " + TempDt.Rows[i]["Item_ID"].ToString() + " And S1.Color_ID = " + TempDt.Rows[i]["Color_ID"].ToString() + " and S1.size_ID = " + TempDt.Rows[i]["Size_ID"].ToString() + " and S3.Rate = " + TempDt.Rows[i]["Rate"].ToString() + " Order by S2.Order_No", ref Dt_Item_OCN[i]);
                }
            }
            catch (Exception ex)
            {
