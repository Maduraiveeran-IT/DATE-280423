...
namespace Accounts
{
    public partial class FrmSocksYarnPOApproval : Form, Entry
    {
        ...
        public void Entry_Edit()
        {
            try
            {
               ...
               ButApp.Text = CmbType.Text;
                ...
                {
                    Dr = Tool.Selection_Tool_Resize(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select PO Details - Reject", "Select S1.PONo, L1.Ledger_Name Supplier, S1.PoDate, S3.Order_No, i1.item, c1.color, s4.size, s2.Order_Qty, S1.RowID, S1.PO_Method, S1.Supplier_Code, S1.Buyer_Code, L2.Ledger_Name Buyer From Socks_Yarn_PO_Master S1 Inner join Socks_Yarn_PO_Details S2 ON S1.RowID = s2.Master_ID Inner join Socks_Order_Master S3 on S2.Order_ID = S3.RowID inner join item I1 on S2.Item_id = I1.itemid inner join color C1 on s2.Color_id = c1.colorid inner join size S4 on s2.Size_ID = S4.sizeid left Join Supplier_All_Fn() L1 On L1.LEdgeR_Code = S1.Supplier_Code  Left Join Buyer_All_Fn() L2 On L2.LEdgeR_code = S1.Buyer_Code LEft Join Socks_Yarn_GRN_OCN_DEtails P1 On S2.RowID = P1.PO_Detail_ID Where  S1.Approval_Flag = 'T' and S1.RowID != 135 and P1.Detail_ID IS Null ", String.Empty, 120, 250, 100, 120, 120, 120, 100, 100);
                }
                else
                {
                    Dr = Tool.Selection_Tool_Resize(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select PO Details - Approval", "Select S1.PONo, L1.Ledger_Name Supplier, S1.PoDate, S3.Order_No, i1.item, c1.color, s4.size, s2.Order_Qty, S1.RowID, S1.PO_Method, S1.Supplier_Code, S1.Buyer_Code, L2.Ledger_Name Buyer From Socks_Yarn_PO_Master S1 Inner join Socks_Yarn_PO_Details S2 ON S1.RowID = s2.Master_ID Inner join Socks_Order_Master S3 on S2.Order_ID = S3.RowID inner join item I1 on S2.Item_id = I1.itemid inner join color C1 on s2.Color_id = c1.colorid inner join size S4 on s2.Size_ID = S4.sizeid left Join Supplier_All_Fn() L1 On L1.LEdgeR_Code = S1.Supplier_Code  Left Join Accounts.Dbo.Ledger_Master L2 on S1.buyer_Code = L2.Ledger_Code and L2.COMPANY_CODE = " + MyParent.CompCode + " and L2.YEAR_CODE = '" + MyParent.YearCode + "' Where  S1.Approval_Flag = 'F' and S1.RowID != 135", String.Empty, 120, 250, 100, 120, 120, 120, 100, 100);
                }
                if (Dr != null)
                {
                    Fill_Data(Dr);
        ...
        public void Entry_Save()
        {
            ...
            try
            {
                ...
                {
                    Queries[Array_Index++] = "update Socks_Yarn_PO_Master Set Supplier_Code = " + TxtSupplier.Tag.ToString() + ", Required_Date = '" + String.Format ("{0:dd-MMM-yyyy}", DtpReqDate.Value) + "' Where RowID = " + Code;
                ...
                else
                {
                    ...
                    for (int i = 0; i <= Dt_Item.Rows.Count - 1; i++)
                    {
                        for (int j = 0; j <= Dt_Item_OCN[i].Rows.Count - 1; j++)
                        {
                            if (MyParent._New)
                            {
                                Queries[Array_Index++] = "Insert into Socks_Yarn_PO_Details (Master_ID, Slno, Order_ID, Item_ID, Color_ID, Size_ID, Order_Qty, Rate, Amount) Values (@@IDENTITY, " + Slno + ", " + Dt_Item_OCN[i].Rows[j]["Order_ID"].ToString() + ", " + Dt_Item.Rows[i]["Item_ID"].ToString() + ", " + Dt_Item.Rows[i]["Color_ID"].ToString() + ", " + Dt_Item.Rows[i]["Size_ID"].ToString() + ", " + Dt_Item_OCN[i].Rows[j]["PO_Qty"].ToString() + ", " + Dt_Item.Rows[i]["Rate"].ToString() + ", " + (Convert.ToDouble(Dt_Item_OCN[i].Rows[j]["PO_Qty"]) * Convert.ToDouble(Dt_Item.Rows[i]["Rate"])) + ")";
                            }
                            else
                            {
                                Queries[Array_Index++] = "Insert into Socks_Yarn_PO_Details (Master_ID, Slno, Order_ID, Item_ID, Color_ID, Size_ID, Order_Qty, Rate, Amount) Values (" + Code + ", " + Slno + ", " + Dt_Item_OCN[i].Rows[j]["Order_ID"].ToString() + ", " + Dt_Item.Rows[i]["Item_ID"].ToString() + ", " + Dt_Item.Rows[i]["Color_ID"].ToString() + ", " + Dt_Item.Rows[i]["Size_ID"].ToString() + ", " + Dt_Item_OCN[i].Rows[j]["PO_Qty"].ToString() + ", " + Dt_Item.Rows[i]["Rate"].ToString() + ", " + (Convert.ToDouble(Dt_Item_OCN[i].Rows[j]["PO_Qty"]) * Convert.ToDouble(Dt_Item.Rows[i]["Rate"])) + ")";
                            }

                            Queries[Array_Index++] = "Update Socks_Yarn_BOM_Status Set PO_Qty = PO_Qty + " + Dt_Item_OCN[i].Rows[j]["PO_Qty"].ToString() + "  Where Order_ID = " + Dt_Item_OCN[i].Rows[j]["Order_ID"].ToString() + " and Item_ID = " + Dt_Item.Rows[i]["Item_ID"].ToString() + " and Color_ID = " + Dt_Item.Rows[i]["Color_ID"].ToString() + " and Size_ID = " + Dt_Item.Rows[i]["Size_ID"].ToString();
                            Slno++;
                        }
                    }
                }
        ...
        public void Entry_Print()
        {
            try
            {
                ...
                Str = " Select S1.PONo, L1.Ledger_Name Supplier, S1.PoDate, S1.Required_Date, (Case When S1.PO_Method = 0 Then 'OCN-WISE' When S1.PO_Method = 0 Then 'ITEM-WISE' End) PO_Method, L1.Ledger_Address Supplier_Address, L1.Ledger_Phone Supplier_Phone, L1.Ledger_email Supplier_Email From Socks_Yarn_PO_Master S1 Left Join Accounts.Dbo.Ledger_Master L1 on S1.Supplier_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Where S1.Approval_Flag = 'T' and S1.RowID = " + Code;
                ...
                Str1 = " Select Top 100000000 ROW_NUMBER()Over(Order By I1.Item + ' - ' + C1.color + ' - ' + S4.Size)SlNo, I1.Item + ' - ' + C1.color + ' - ' + S4.Size Item_Color_Size, S2.Order_Qty, S2.Rate, S2.Amount From Socks_Yarn_PO_Master S1 Inner join Socks_Yarn_PO_Details S2 ON S1.RowID = s2.Master_ID Inner join Socks_Order_Master S3 on S2.Order_ID = S3.RowID Inner join item I1 on S2.Item_id = I1.itemid Inner join color C1 on s2.Color_id = c1.colorid Inner join size S4 on s2.Size_ID = S4.sizeid Where S1.RowID = " + Code + " Order By I1.Item + ' - ' + C1.color + ' - ' + S4.Size ";
                MyBase.Execute_Qry(Str1, "Socks_Yarn_PO");

                Str2 = " Select Top 2 S1.Slno Sl, S1.Tax_Code, L1.Ledger_Name Tax, S1.Mode Tax_Mode, S1.Tax_Per, S1.Tax_Amount, '' T From Socks_Yarn_Tax_Details S1 Left Join Accounts.dbo.Ledger_Master L1 on S1.Tax_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Where S1.Master_ID = " + Code + " Order by S1.Slno ";
        ...
        void Load_OCN()
        {
            try
            {
                //if (MyParent._New)
                //{
                //    Grid_OCN.DataSource = MyBase.Load_Data("Select S1.RowID, S1.Order_No From SocksPO_Pending_OCN (" + TxtBuyer.Tag.ToString() + ") S1 Inner join Socks_Time_Action_PO_Complete_Orders () S2 on S1.Order_No = S2.Order_No Where S2.Complete_Date_End <= Cast(GetDate() as Date) Order by S1.Order_NO", ref Dt_OCN);
                //}
                //else
                //{
                    Grid_OCN.DataSource = MyBase.Load_Data("Select S2.RowID, S2.Order_No From Socks_Yarn_PO_OCN_Details S1 Inner join Socks_Order_Master S2 on S1.Order_ID = S2.RowID Where S1.Master_ID = " + Code + " order by S2.Order_NO", ref Dt_OCN);
                //}
        ...
        private void FrmSocksYarnPOApproval_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void FrmSocksYarnPOApproval_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        void Load_Item()
        {
            try
            {
                //if (MyParent._New)
                //{
                //    Grid_Item.DataSource = MyBase.Load_Data("Select 0 as Sl, Size + ' ' + Item + ' ' + Color + ' @ ' + Cast(Rate as Varchar (15)) Description, Item_ID, Item ITEM, Color_ID, Color COLOR, Size_ID, size SIZE, Sum(BOM_Qty) BOM_Qty, Sum(Ordered_Qty) Ordered_Qty, Sum(PO_Qty) Bal_Qty, Sum(PO_Qty) PO_Qty, RATE ARATE, RATE, SUM(Amount) AMOUNT From SocksPO_Pending_Items (5275) where 1 = 2 Group by Item_ID, Item, Color_ID, Color, Size_ID, size, Rate", ref Dt_Item);
                //}
                //else
                //{
                    // Also Change in Load_Dt_Item_OCN for Edit
                    Grid_Item.DataSource = MyBase.Load_Data("Select 0 as Sl, S2.Size + ' ' + I1.Item + ' ' + C1.Color + ' @ ' + Cast(S1.Rate as Varchar (15)) Description, S1.Item_ID, I1.Item ITEM, S1.Color_ID, C1.Color COLOR, S1.Size_ID, S2.size SIZE, Sum(S1.BOM_Qty) BOM_Qty, (Sum(S1.Ordered_Qty) - Sum(S3.Order_Qty)) Ordered_Qty, (Sum(S1.BOM_Qty) - (Sum(S1.Ordered_Qty) - Sum(S3.Order_Qty))) Bal_Qty, Sum(S3.Order_Qty) PO_Qty, S1.RATE ARATE, S3.RATE, SUM(S3.Amount) AMOUNT From SocksPO_Pending_Items_All() S1 Inner Join Socks_Yarn_PO_Details S3 on S1.Order_ID = S3.Order_ID and S1.Item_ID = S3.Item_id and S1.Color_ID = S3.Color_id and S1.Size_ID = S3.Size_ID  Inner Join item I1 on S1.Item_id = I1.itemid Inner Join color C1 on s1.Color_id = c1.colorid Inner Join size S2 on s1.Size_ID = S2.sizeid Where Master_ID = " + Code + " Group By S1.Item_ID, I1.Item, S1.Color_ID, C1.Color, S1.Size_ID, S2.size, S1.Rate, S3.Rate", ref Dt_Item);
                //}
        ...
        private void FrmSocksYarnPOApproval_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if (this.ActiveControl is TextBox)
        ...
        void Load_Tax()
        {
            try
            {
                ...
                //else
                //{
                    Grid_Tax.DataSource = MyBase.Load_Data("Select S1.Slno Sl, S1.Tax_Code, L1.Ledger_Name Tax, S1.Mode Tax_Mode, S1.Tax_Per, S1.Tax_Amount, '' T From Socks_Yarn_Tax_Details S1 Left Join Accounts.dbo.Ledger_Master L1 on S1.Tax_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Where S1.Master_ID = " + Code + " Order by S1.Slno ", ref Dt_Tax);
                //}
            ...
            try
            {
                ...
                return;
                    if (Grid_Item.CurrentCell.ColumnIndex == Grid_Item.Columns["Item"].Index)
                    {
                        ...
                        if (TxtBuyer.Text.Trim() == String.Empty)
                        {
                            Dr = Tool.Selection_Tool_Except_New("Description", this, 30, 70, ref Dt_Item, SelectionTool_Class.ViewType.NormalView, "Select Item", "Select S1.Size + ' ' + S1.Item + ' ' + S1.Color + ' @ ' + Cast(S1.Rate as Varchar (15)) Description, Sum(S1.BOM_Qty) Bom_Qty, Sum(S1.Ordered_Qty) Ordered_Qty, Sum(S1.PO_Qty) PO_Qty, S1.Rate, SUM(S1.Amount) Amount, S1.Item_ID, S1.Item, S1.Color_ID, S1.Color, S1.Size_ID, S1.size From SocksPO_Pending_Items_All () S1 Inner join Socks_Time_Action_PO_Complete_Orders () S2 on S1.Order_No = S2.Order_No Where S2.Complete_Date_End <= Cast(GetDate() as Date) And S1.Color_ID <> 867 Group by S1.Item_ID, S1.Item, S1.Color_ID, S1.Color, S1.Size_ID, S1.size, S1.Rate Having Sum(S1.PO_Qty) > 0 ", String.Empty, 250, 80, 80, 80);
                        }
                        else
                        {
                            Dr = Tool.Selection_Tool_Except_New("Description", this, 30, 70, ref Dt_Item, SelectionTool_Class.ViewType.NormalView, "Select Item", "Select S1.Size + ' ' + S1.Item + ' ' + S1.Color + ' @ ' + Cast(S1.Rate as Varchar (15)) Description, Sum(S1.BOM_Qty) Bom_Qty, Sum(S1.Ordered_Qty) Ordered_Qty, Sum(S1.PO_Qty) PO_Qty, S1.Rate, SUM(S1.Amount) Amount, S1.Item_ID, S1.Item, S1.Color_ID, S1.Color, S1.Size_ID, S1.size From SocksPO_Pending_Items (" + TxtBuyer.Tag.ToString() + ") S1 Inner join Socks_Time_Action_PO_Complete_Orders () S2 on S1.Order_No = S2.Order_No Where S2.Complete_Date_End <= Cast(GetDate() as Date) And S1.Color_ID <> 867 Group by S1.Item_ID, S1.Item, S1.Color_ID, S1.Color, S1.Size_ID, S1.size, S1.Rate Having Sum(S1.PO_Qty) > 0 ", String.Empty, 250, 80, 80, 80);
                        }
                        if (Dr != null)
                        {
                            MyBase.Row_Number(ref Grid_Item);
        ...
        void Load_Dt_Item_OCN()
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= TempDt.Rows.Count - 1; i++)
                {
                    ...
                    MyBase.Load_Data("Select 0 as Sl, S1.Order_ID, S2.Order_No ORDER_NO, L1.Ledger_Name BUYER, S1.BOM_Qty, (S1.Ordered_Qty - S3.Order_Qty) Ordered_Qty, (S1.BOM_Qty - (S1.Ordered_Qty - S3.Order_Qty)) Bal_Qty, S3.Order_Qty PO_Qty, '' T From SocksPO_Pending_Items_All() S1 Inner Join Socks_Order_Master S2 on S1.Order_ID = S2.RowID Inner Join Socks_Yarn_PO_Details S3 On S1.Order_ID = S3.Order_ID and  S1.Item_ID = S3.Item_id and S1.Color_ID = S3.Color_id and S1.Size_ID = S3.Size_ID Left Join Accounts.Dbo.Ledger_Master L1 on S2.Party_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Where S3.Master_ID = " + Code + " and S1.Item_ID = " + TempDt.Rows[i]["Item_ID"].ToString() + " And S1.Color_ID = " + TempDt.Rows[i]["Color_ID"].ToString() + " and S1.size_ID = " + TempDt.Rows[i]["Size_ID"].ToString() + " and S3.Rate = " + TempDt.Rows[i]["Rate"].ToString() + " Order by S2.Order_No", ref Dt_Item_OCN[i]);
                }
            }
            catch (Exception ex)
            {
        ...
        void load_grid_item_ocn(Int32 Parent_Row)
        {
            ...
            try
            {
                ...
                if (Dt_Item_OCN[Parent_Row] == null)
                {
                    ...
                    {
                        MyBase.Load_Data("Select 0 as Sl, S1.Order_ID, S2.Order_No ORDER_NO, L1.Ledger_Name BUYER, S1.Bom_Qty, S1.Ordered_Qty, S1.PO_Qty Bal_Qty, Cast(0 as Numeric (25, 3)) As Po_Qty, '' T From SocksPO_Pending_Items_All() S1 Inner join Socks_Order_Master S2 on S1.Order_ID = s2.RowID Left join Accounts.Dbo.ledger_Master L1 on S2.Party_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Inner Join Socks_Time_Action_PO_Complete_Orders () S5 on S2.Order_No = S5.Order_No Where S5.Complete_Date_End <= Cast(GetDate() as Date) And S1.Item_ID = " + Grid_Item["Item_ID", Parent_Row].Value.ToString() + " and S1.Color_ID = " + Grid_Item["Color_ID", Parent_Row].Value.ToString() + " and S1.size_ID = " + Grid_Item["Size_ID", Parent_Row].Value.ToString() + " and Rate = " + Grid_Item["ARATE", Parent_Row].Value.ToString() + " order by S2.Order_No", ref Dt_Item_OCN[Parent_Row]);
                    }
                    else
                    {
                        MyBase.Load_Data("Select 0 as Sl, S1.Order_ID, S2.Order_No ORDER_NO, L1.Ledger_Name BUYER, S1.Bom_Qty, S1.Ordered_Qty, S1.PO_Qty Bal_Qty, Cast(0 as Numeric (25, 3)) As Po_Qty, '' T From SocksPO_Pending_Items(" + TxtBuyer.Tag.ToString() + ") S1 Inner join Socks_Order_Master S2 on S1.Order_ID = s2.RowID Left join Accounts.Dbo.ledger_Master L1 on S2.Party_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "'  Inner Join Socks_Time_Action_PO_Complete_Orders () S5 on S2.Order_No = S5.Order_No Where S5.Complete_Date_End <= Cast(GetDate() as Date) And S1.Item_ID = " + Grid_Item["Item_ID", Parent_Row].Value.ToString() + " and S1.Color_ID = " + Grid_Item["Color_ID", Parent_Row].Value.ToString() + " and S1.size_ID = " + Grid_Item["Size_ID", Parent_Row].Value.ToString() + " and Rate = " + Grid_Item["ARATE", Parent_Row].Value.ToString() + " order by S2.Order_No", ref Dt_Item_OCN[Parent_Row]);
                        TxtBuyer.Enabled = false;
                    }
                }
        ...
        private void ButApp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if(CmbType.SelectedIndex == 1)
                {
                    MyBase.Run("Update Socks_Yarn_PO_Master Set Approval_Flag = 'F' , Approval_System = Host_Name(), Approval_Time = Getdate() Where RowID = " + Code + " ");
                }
                else
                {
                    MyBase.Run("Update Socks_Yarn_PO_Master Set Approval_Flag = 'T' , Approval_System = Host_Name(), Approval_Time = Getdate() Where RowID = " + Code + " ");
                }
