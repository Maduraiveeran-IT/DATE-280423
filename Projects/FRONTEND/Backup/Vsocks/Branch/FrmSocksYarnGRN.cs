    ...
    public partial class FrmSocksYarnGRN : Form, Entry
    {
        ...
        public void Entry_New()
        {
            try
            {
                ...
                Dt_OCN_New = new DataTable[20, 20, 2];
                TxtSupplier.Focus();
            }
            catch (Exception ex)
            {
        ...
        void Load_OCN_Lot_EDIT()
        {
            try
            {
                for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                {
                    ...
                    MyBase.Load_Data("Select S1.Slno SL, (S2.Order_No + '-' + S8.PONo) Description, S1.RowID OCN_RowID, S1.Order_ID, S1.PO_Detail_ID, S2.Order_No, S8.PONo, (S9.Bal_Qty + S1.Qty) PO_QTY, S1.Qty GRN_QTY, '' T From Socks_Yarn_GRN_OCN_DEtails S1 left Join Socks_Order_Master S2 on S1.Order_ID = S2.RowID Inner Join Socks_Yarn_PO_Details S7 on S1.PO_Detail_ID = S7.RowID Inner Join Socks_Yarn_PO_Master S8 on S7.Master_ID = S8.RowID Left join Socks_Yarn_GRN_Pending_OCN() S9 on S7.Item_id = S9.Item_id and S7.Color_id = S9.Color_id and S7.Size_ID = S9.Size_ID and S1.Order_ID = S9.Order_ID and S9.PO_Detail_ID = S7.RowID Where S1.Detail_ID = " + Dt.Rows[i]["Detail_ID"].ToString(), ref Dt_OCN_New[i, 0, 0]);

                    for (int j = 0; j <= Dt_OCN_New[i, 0, 0].Rows.Count - 1; j++)
                    {
                        Dt_OCN_New[i, j, 1] = new DataTable();
                        MyBase.Load_Data("Select S1.Slno SL, S1.Lot_No, S1.Bag_No, S1.Qty, S1.Location_ID, S3.Location, '' T From Socks_Yarn_GRN_OCN_Lot_Details S1 Left Join Socks_Yarn_Stores_location_Master S3 on S1.Location_ID = S3.rowID Where S1.OCN_RowID = " + Dt_OCN_New [i, 0, 0].Rows[j]["OCN_RowID"].ToString() , ref Dt_OCN_New[i, j, 1]);
                    }
                }
            }
            catch (Exception ex)
        ...
        public void Entry_Save()
        {
            ...
            try
            {
                ...
                MyBase.Load_Data("Select No, Date FRom Socks_Grn_Gatepass_UnLock Where No = '" + TxtSupplier.Tag.ToString() + "' and Mode = 'Internal' ", ref Tdts);
                if(Tdts.Rows.Count > 0)
                {
                    TxtGatePass.Text = TxtGRNNo.Text.ToString().Substring(7,5);
                    DtpGPDate.Value = DtpDate.Value;
                    ...
                    if (Tdt1.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i <= Tdt1.Rows.Count - 1; i++)
                        {
                            if (CQty > 0)
                            {
                                ...
                                {
                                    MyBase.SQLCmd.CommandText = "Update Socks_Yarn_Bom_Status Set Grn_Qty = GRN_Qty - " + Convert.ToDouble(Tdt1.Rows[i]["GRN_Qty"]) + " Where Planning_Detail_ID = " + Tdt1.Rows[i]["Planning_Detail_ID"].ToString();
                                    MyBase.SQLCmd.ExecuteNonQuery();
                                    CQty = CQty - Convert.ToDouble(Tdt1.Rows[i]["GRN_Qty"]);
                                }
                                else
                                {
                                    MyBase.SQLCmd.CommandText = "Update Socks_Yarn_Bom_Status Set Grn_Qty = GRN_Qty - " + CQty + " Where Planning_Detail_ID = " + Tdt1.Rows[i]["Planning_Detail_ID"].ToString();
                                    MyBase.SQLCmd.ExecuteNonQuery();
                                    CQty = 0;
                                }
                            }
                ...
                for (int i=0;i<=Dt.Rows.Count - 1;i++)
                {
                    ...
                    for (int j=0;j<= Dt_OCN_New[i, 0, 0].Rows.Count - 1;j++)
                    {
                        ...
                        if (Convert.ToInt64(Grid["Color_ID", i].Value) == 867)
                        {
                            //MyBase.SQLCmd.CommandText = "Exec Update_Greige_GRN " + Convert.ToDouble(Dt_OCN_New[i, 0, 0].Rows[j]["GRN_Qty"]) + ", " + Dt_OCN_New[i, 0, 0].Rows[j]["Order_ID"].ToString() + ", " + Grid["Item_ID", i].Value.ToString() + ", " + Grid["Color_ID", i].Value.ToString() + ", " + Grid["Size_ID", i].Value.ToString();
                            // Greige Purpose Only. /// 31/08/2016
                            Double CQty = Convert.ToDouble(Dt_OCN_New[i, 0, 0].Rows[j]["GRN_Qty"]);
                            DataTable Tdt1 = new DataTable();
                            MyBase.Load_Data("Select Planning_Detail_ID, (PO_Qty - GRN_Qty) Bal_Qty From Socks_Yarn_bom_Status Where Order_ID = " + Dt_OCN_New[i, 0, 0].Rows[j]["Order_ID"].ToString() + " and Item_ID = " + Grid["Item_ID", i].Value.ToString() + " and Color_ID = " + Grid["Color_ID", i].Value.ToString() + " and Size_ID = " + Grid["Size_ID", i].Value.ToString() + " and (PO_Qty - GRN_Qty) > 0 and " + Dt_OCN_New[i, 0, 0].Rows[j]["Order_ID"].ToString() + " != 'Y' Order By Planning_Detail_ID", ref Tdt1);
                            for (int m = 0; m <= Tdt1.Rows.Count - 1; m++)
                            {
                                if (CQty > Convert.ToDouble(Tdt1.Rows[m]["Bal_Qty"]))
                                {
                                    MyBase.SQLCmd.CommandText = "update Socks_Yarn_BOM_Status Set GRN_Qty = GRN_Qty + " + Convert.ToDouble(Tdt1.Rows[m]["Bal_Qty"]) + " Where Planning_Detail_ID = " + Tdt1.Rows[m]["Planning_Detail_ID"].ToString();
                                    MyBase.SQLCmd.ExecuteNonQuery();
                                    CQty = CQty - Convert.ToDouble(Tdt1.Rows[m]["Bal_Qty"]);
                                }
                                else
                                {
                                    MyBase.SQLCmd.CommandText = "update Socks_Yarn_BOM_Status Set GRN_Qty = GRN_Qty + " + CQty + " Where Planning_Detail_ID = " + Tdt1.Rows[m]["Planning_Detail_ID"].ToString();
                                    MyBase.SQLCmd.ExecuteNonQuery();
                                    CQty = 0;
                                }
                            }
        ...
        public void Entry_View()
        {
            try
            {
                MyBase.Clear(this);
                MyBase.Enable_Controls(this, true);
                Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select GRN - View", "Select S1.GRNNo, S1.GRNDate, L1.Ledger_Name Supplier, S1.Net_Amount, I1.Item, C1.Color, S3.size, S2.GRN_Qty, S2.Rate, S1.RowID Code, S1.GP_NO, S1.GP_Date, S1.DC_No, S1.DC_Date, S1.Invoice_No, S1.Invoice_Date, S1.Supplier_Code From Socks_Yarn_GRN_Master S1 Inner join Socks_Yarn_GRN_Details S2 on S1.RowID = S2.Master_ID Inner join item I1 on S2.Item_ID = I1.itemid inner join size S3 on S2.Size_ID = S3.sizeid inner join color C1 on S2.Color_ID = C1.colorid Left join Dbo.Supplier_All_Fn() L1 on S1.Supplier_Code = L1.Ledger_Code ", String.Empty, 120, 90, 250, 100, 120, 120, 120, 100, 100);
                if (Dr != null)
                {
                    Fill_Datas(Dr);
                }
        ...
        void Load_Item()
        {
            try
            {
                ...
                if (MyParent._New)
                {
                    Grid.DataSource = MyBase.Load_Data("Select 0 as SL, Cast(0 as Bigint) Detail_ID, Item + ' ' + Color + ' ' + Size + ' @ ' + Cast(Rate as Varchar (15)) Description, Item_ID, Item ITEM, Color_ID, Color COLOR, Size_ID, Size SIZE, PO_Qty, Inward_Qty, Bal_Qty, Bal_Qty GRN_Qty, Rate, Cast(0 as Numeric (25, 2)) Amount From Socks_Yarn_GRN_Pending () Where 1=2", ref Dt);
                }
                else
                {
                    Grid.DataSource = MyBase.Load_Data("Select S2.Slno SL, S2.RowID Detail_ID, S1.Item + ' ' + S1.Color + ' ' + S1.Size + ' @ ' + Cast(S1.Rate as Varchar (15)) Description, S1.Item_ID, S1.Item ITEM, S1.Color_ID, S1.Color COLOR, S1.Size_ID, S1.Size SIZE, S1.PO_Qty, (S1.Inward_Qty) Inward_Qty, (S1.Bal_Qty + S2.GRN_Qty) Bal_Qty, S2.GRN_Qty, S2.Rate, (S2.Rate * S2.GRN_Qty) Amount From Socks_Yarn_GRN_Pending () S1 Inner join Socks_Yarn_Grn_Details S2 on S1.Item_id = S2.Item_ID and S1.Color_id = S2.Color_ID and S1.Size_ID = S2.Size_ID And S1.Rate = S2.Rate Inner Join Socks_Yarn_GRN_Master S3 On S2.Master_ID = S3.RowID and S1.Supplier_Code = S3.Supplier_Code  Where S2.Master_ID = " + Code + " Order By S2.Slno", ref Dt);
                }
            ...
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    if (this.ActiveControl.Name == "TxtSupplier")
                    {
                        ...
                        { 
                            Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Supplier", "Select Distinct L1.Ledger_Name Supplier, S1.SUpplier_Code From Socks_Yarn_GRN_Pending () S1 Left Join Supplier_All_Fn() L1 on S1.Supplier_Code = L1.Ledger_Code Where S1.Supplier_Code != 793", String.Empty, 350);
                            if (Dr != null)
                            {
                                TxtSupplier.Tag = Dr["Supplier_Code"].ToString();
                                TxtSupplier.Text = Dr["Supplier"].ToString();
        ...
        void Load_Lot(Int32 Row)
        {
            try
            {
                if (Dt_OCN_New[Grid.CurrentCell.RowIndex, Row, 1] == null)
                {
                    ...
                    MyBase.Load_Data("Select S1.Slno SL, S1.Lot_No, S1.Bag_No, S1.Qty, S1.Location_ID, S3.Location, '' T From Socks_Yarn_GRN_OCN_Lot_Details S1 Left Join Socks_Yarn_Stores_location_Master S3 on S1.Location_ID = S3.rowID Where 1 = 2", ref Dt_OCN_New[Grid.CurrentCell.RowIndex, Row, 1]);
                }
        ...
        void Load_Tax()
        {
            try
            {
                if (MyParent._New)
                {
                    Grid_Tax.DataSource = MyBase.Load_Data("Select S1.Slno Sl, S1.Tax_Code, L1.Ledger_Name Tax, S1.Mode Tax_Mode, S1.Tax_Per, S1.Tax_Amount, '' T From Socks_Yarn_GRN_Tax_Details S1 Left Join Accounts.dbo.Ledger_Master L1 on S1.Tax_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Where 1 = 2 Order by S1.Slno ", ref Dt_Tax);
                }
                else
                {
                    Grid_Tax.DataSource = MyBase.Load_Data("Select S1.Slno Sl, S1.Tax_Code, L1.Ledger_Name Tax, S1.Mode Tax_Mode, S1.Tax_Per, S1.Tax_Amount, '' T From Socks_Yarn_GRN_Tax_Details S1 Left Join Accounts.dbo.Ledger_Master L1 on S1.Tax_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Where S1.Master_ID = " + Code + " Order by S1.Slno ", ref Dt_Tax);
                }
        ...
        void Txt_OCN_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Down)
                {
                    ...
                    {                        
                       // Dr = Tool.Selection_Tool_Except_New("Description", this, 30, 70, ref Dt_OCN_New[Grid.CurrentCell.RowIndex, 0, 0], SelectionTool_Class.ViewType.NormalView, "Select OCN", "Select Distinct S2.Order_No, S1.PONo, S1.Bal_Qty PO_Qty, S1.Order_ID, S1.PO_Detail_ID, (S2.Order_No + '-' + S1.PONo) Description From Socks_Yarn_GRN_Pending_OCN () S1 left join Socks_Order_Master S2 on S1.Order_ID = S2.RowID Where S1.Bal_Qty > 0 and Item_ID = " + Grid["Item_ID", Grid.CurrentCell.RowIndex].Value.ToString() + " and Color_ID = " + Grid["Color_ID", Grid.CurrentCell.RowIndex].Value.ToString() + " and Size_ID = " + Grid["Size_ID", Grid.CurrentCell.RowIndex].Value.ToString() + " and Rate = " + Grid["Rate", Grid.CurrentCell.RowIndex].Value.ToString(), String.Empty, 120, 120, 100);
                        MyBase.Row_Number(ref Grid_OCN);
                        ...
                        if (Dr != null)
                        {
                            ...
                            //       MyBase.Load_Data("Select ROW_NUMBER() Over (Order by From_Order_No) SL, From_Order_No + 'GUP-POY00000' Description, 0 OCN_RowID, 0 Order_ID, 135 PO_Detail_ID, From_Order_No OrdeR_No, 'GUP-POY00000' PoNo, (Case When Qty > To_Be_Rcvd Then 0.100 Else Qty End) PO_Qty, 0.000 GRN_QTY, 'Y' T  FRom Socks_GRN_Required_OCN_With_Item('" + Dr["order_NO"].ToString() + "', " + Grid["Item_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ", " + Grid["Color_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ", " + Grid["Size_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ")", ref TDtP);
                            //       if(TDtP.Rows.Count >0)
                            //       {
                            //           Dt_OCN_New[Grid.CurrentCell.RowIndex, 0, 0] = TDtP.Copy();
                            //           Grid_OCN.DataSource = Dt_OCN_New[Grid.CurrentCell.RowIndex, 0, 0];
                            //           return;
                            //       }                                
                            //}
                            Grid_OCN["Description", Grid_OCN.CurrentCell.RowIndex].Value = Dr["Description"].ToString();
