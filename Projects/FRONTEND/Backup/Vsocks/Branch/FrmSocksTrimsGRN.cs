    ...
    public partial class FrmSocksTrimsGRN : Form, Entry
    {
            ...
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    if (this.ActiveControl.Name == "TxtSupplier")
                    {
                        ...
                        {
                            Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Supplier", "Select Distinct L1.Ledger_Name Supplier, S1.SUpplier_Code From Socks_Trims_GRN_Pending () S1 Left Join Accounts.Dbo.Ledger_Master L1 on S1.Supplier_Code = L1.Ledger_Code and L1.Company_Code = " + MyParent.CompCode + " and L1.Year_Code = '" + MyParent.YearCode + "' Where S1.Supplier_Code != 793", String.Empty, 350);
                            if (Dr != null)
                            {
                                TxtSupplier.Tag = Dr["Supplier_Code"].ToString();
                                TxtSupplier.Text = Dr["Supplier"].ToString();
        ...
        void Load_Tax()
        {
            try
            {
                if (MyParent._New)
                {
                    Grid_Tax.DataSource = MyBase.Load_Data("Select S1.Slno Sl, S1.Tax_Code, L1.Ledger_Name Tax, S1.Mode Tax_Mode, S1.Tax_Per, S1.Tax_Amount, '' T From Socks_Trims_GRN_Tax_Details S1 Left Join Accounts.dbo.Ledger_Master L1 on S1.Tax_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Where 1 = 2 Order by S1.Slno ", ref Dt_Tax);
                }
                else
                {
                    Grid_Tax.DataSource = MyBase.Load_Data("Select S1.Slno Sl, S1.Tax_Code, L1.Ledger_Name Tax, S1.Mode Tax_Mode, S1.Tax_Per, S1.Tax_Amount, '' T From Socks_Trims_GRN_Tax_Details S1 Left Join Accounts.dbo.Ledger_Master L1 on S1.Tax_Code = L1.Ledger_Code and L1.COMPANY_CODE = " + MyParent.CompCode + " and L1.YEAR_CODE = '" + MyParent.YearCode + "' Where S1.Master_ID = " + Code + " Order by S1.Slno ", ref Dt_Tax);
                }
