...
namespace Accounts
{
    public partial class FrmSocks_Knitting_QC_Entry : Form, Entry
    {
        ...
        public FrmSocks_Knitting_QC_Entry()
        {
            this.InitializeComponent();
        }

        private void FrmSocks_Knitting_QC_Entry_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        public void Entry_Save()
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                {
                    ...
                    if (Grid["QCQty", i].Value == DBNull.Value || Grid["QCQty", i].Value.ToString() == String.Empty || Convert.ToDouble(Grid["QCQty", i].Value) == 0)
                    {
        ...
        private void FrmSocks_Knitting_QC_Entry_KeyDown(object sender, KeyEventArgs e)
       {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    else if (ActiveControl.Name == "TxtRemarks")
                    {
                        TxtTotal.Focus();  
                    }
                    else
        ...
        private void FrmSocks_Knitting_QC_Entry_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
        ...
        void Grid_Data(Boolean Buffer)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (MyParent._New)
                    {
                        Str = "Select 0 as Slno, Q1.MachineID Machine, Q1.NeedleID, Q1.NeedleID Needle, Q1.Order_No, S2.Buyer, Q1.OrderQty, Q1.ItemID, Cast('' As Varchar (15)) Sample, S2.im_Color Color, Q1.OrderColorID, Q1.SizeID, S1.Size, Q1.BOMQty Bom, Cast(0 as Bigint) Bal_QC, Cast(0.00 as Numeric(6,2)) QCQty, Q1.Problem_ID, Cast('' as varchar)Problem, Q1.Emplno_OPerator, E1.Name OPerator, '-' T From Socks_QC_Master Q2 Left Join Socks_QC_Details Q1 On Q2.RowID = Q1.MasterID Left Join VFit_Sample_Needle_Master V2 On Q1.NeedleID = V2.RowID Left Join size S1 on Q1.SizeID = S1.sizeid Left Join Vaahini_ERP_Gainup.Dbo.Employeemas E1 on Q1.Emplno_Operator = E1.Emplno Left Join Socks_Bom() S2 On Q1.Order_No = S2.Order_No And Q1.OrderColorID = S2.OrderColorId And Q1.SizeID = S2.sizeid Where 1 = 2";
                    }
                    else
                    {
                ...
                MyBase.Grid_Width(ref Grid, 40, 60, 60, 120, 120, 100, 100, 100, 80, 80, 140, 120);
                ...
                Grid.Columns["QCQty"].DefaultCellStyle.Format = "0.00";
            }
            catch (Exception ex)
            {
                throw ex;
                ...
                else if (Grid.CurrentCell.ColumnIndex == Grid.Columns["Order_No"].Index && Grid.CurrentCell.Value.ToString() == String.Empty)
                {
                    ...
                    if (Grid["Machine", Grid.CurrentCell.RowIndex].Value.ToString() != String.Empty)
                    {
                        if (Grid.CurrentCell.RowIndex > 0)
                        {
                            ...
                            Grid["Buyer", Grid.CurrentCell.RowIndex].Value = Grid["Buyer", Grid.CurrentCell.RowIndex - 1].Value.ToString();
                            Grid["Sample", Grid.CurrentCell.RowIndex].Value = Grid["Sample", Grid.CurrentCell.RowIndex - 1].Value.ToString();
                            Grid["Color", Grid.CurrentCell.RowIndex].Value = Grid["Color", Grid.CurrentCell.RowIndex - 1].Value.ToString();
        ...
        void Problem_Selection()
        {
            try
            {
                Dr = Tool.Selection_Tool(this, 150, 150, SelectionTool_Class.ViewType.NormalView, "Select Problem", "Select Name Problem, Type, RowID From Socks_QC_Problem_Master Where Type like 'KNITTING'", String.Empty, 150, 150);
                if (Dr != null)
                {
        ...
        void OrderNo_Selection()
        {
            try
            {
                Dr = Tool.Selection_Tool(this, 150, 150, SelectionTool_Class.ViewType.NormalView, "Select Order", "Select S1.Order_No, S1.Buyer, S1.Color Sample, S1.im_Color Color, S1.SizeID, S1.Size, S1.Bom_Qty Bom, ISNULL(CAST(S1.AllowancePer as Varchar),'NOT AVAILABLE') Allowance, (S1.Bom_Qty - Isnull(K1.Tested, 0)) Balance_Testing, S1.Order_Qty, S1.ItemID, S1.OrderColorID, S3.bsm_needle Needle From Socks_Bom () S1 Left Join Socks_QC_Knitting_All() k1 on S1.Order_No = K1.OrderNo and S1.OrderColorId = K1.OrderColorID and S1.sizeid = K1.SizeID Left Join Stage_Sample_Master S3 On S1.color = S3.bsm_f_item_code ", String.Empty, 100, 100, 70, 80, 60, 50, 60, 80, 80, 80, 80);
                if (Dr != null)
                {
                    if (Dr["Bom"].ToString() == String.Empty || Dr["Bom"].ToString() == null)
                    {
                        ...
                        Txt.Text = "";
                        Grid["Buyer", Grid.CurrentCell.RowIndex].Value = "";
                        Grid["Sample", Grid.CurrentCell.RowIndex].Value = "";
                        Grid["Color", Grid.CurrentCell.RowIndex].Value = "";
                        ...
                        Grid["NeedleID", Grid.CurrentCell.RowIndex].Value = "";
                        Grid["Needle", Grid.CurrentCell.RowIndex].Value = "";
                    }
                    else
                    {
        ...
        void Operator_Selection()
        {
            try
            {
                ...
                Dr = Tool.Selection_Tool(this, 150, 150, SelectionTool_Class.ViewType.NormalView, "Select Operator", "Select Name, Tno, Emplno From Vaahini_ERP_Gainup.Dbo.Employeemas E1 Inner Join Vaahini_ERP_Gainup.Dbo.Depttype D1 on E1.Deptcode = D1.DeptCode and E1.COMPCODE = D1.compcode Where E1.compcode = 2 and D1.deptCode = 82 and E1.tno not like '%Z'", String.Empty, 250, 80);
        ...
        
        {
            try
            {
                if (Grid.CurrentCell != null && Mode != 1)
                {
