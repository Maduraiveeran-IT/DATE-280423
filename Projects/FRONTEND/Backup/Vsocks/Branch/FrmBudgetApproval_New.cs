...
namespace Accounts
{
    public partial class FrmBudgetApproval_New : Form
    {
        ...
        public FrmBudgetApproval_New()
        {
            InitializeComponent();
        }

        private void FrmBudgetApproval_New_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    if (this.ActiveControl.Name == "TxtBuyer")
                    {
                        ...
                        Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Buyer", "Select Distinct B.Party Buyer, B.Party_Code BuyerID From FitSocks.Dbo.Socks_Planning_Master  A Inner Join FitSocks.Dbo.Socks_Order_Fn() B On A.Order_ID = B.RowID and A.Item_ID = B.ItemID Order by B.Party ", String.Empty, 350);                       
                        if (Dr != null)
                        {
        ...
        void Grid_Data()
        {
            ...
            try
            {                
                Str = "Select A.Order_No, A.Item Style, A.Buyer_Qty_Plan Qty, Max(B.Ship_Date) Ship_Date, B.Currency , B.Ex_Rate Price  From FitSocks.Dbo.Socks_Bom_Item_Fn() A Inner Join FitSocks.Dbo.Socks_Order_Fn() B On A.Order_ID = B.RowID and A.ItemID = B.ItemID Inner Join FitSocks.Dbo.Socks_Planning_Master C On A.Order_ID = C.Order_ID and A.ItemID = C.Item_ID Where A.Party_Code = " + TxtBuyer.Tag.ToString() + " Group by A.Order_No, A.Item, A.Buyer_Qty_Plan , B.Currency , B.Ex_Rate Order by A.Order_No Desc";
        ...
        private void FrmBudgetApproval_New_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
        ...
        private void FrmBudgetApproval_New_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        void Summary()
        {
            ...
            try
            {
                Mybase.Load_Data("SELECT 0 AS SLNO, O_Slno, Access_Type Access, Cast(0 as Numeric (25, 2)) PREVIOUS, Cast(0 as Numeric (25, 2)) as NOW_, Cast(0 as Numeric (25, 2)) AMOUNT FROM Vaahini_ERP_Gainup.DBo.Budget_Approval_Summary_Socks_New() WHERE ORDER_NO in (" + OCN_List + ") Group By O_Slno, Access_Type Order By O_Slno", ref Dt_Sum);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Query += " Set @Result = ' Select O_Slno, Access_Type ACCESS, Processid, ItemID, ColorID, SizeID, PROCESS, Item ITEM, Color COLOR, Size SIZE, ' + @Ocn_No_List_Isnull + ' FROM (Select Order_No, O_Slno, Access_Type, Processid, ItemID, ColorID, SizeID, PROCESS, Item, Color, Size, Rate From Vaahini_ERP_Gainup.DBo.Budget_Approval_Socks_New() where order_no IN (' + @Ocn_No_List_SQ + ')) A1 PIVOT (Sum(RATE) for order_no in (' + @Ocn_No_List_Act + ')) A '; Exec SP_ExecuteSql @Result;";
                ...
                for (int i = 0; i <= Dt_Budget.Rows.Count - 1; i++)
                {
                    for (int k = 10; k <= Dt_Budget.Columns.Count - 1; k++)
                    {
                        if (Convert.ToInt32(Dt_Budget.Rows[i]["O_Slno"]) == 0)
                        {
                            ...
                            {
                                Queries[Array_Index++] = "Insert Into FitSocks.Dbo.Socks_Yarn_BOM_Status (Planning_Master_ID, Planning_Detail_ID, Order_ID, Item_ID, Color_ID, Size_ID, BOM, Dyeing_Status)  Select Top 1000000000 RowID, PlanDtlID, OrdeR_ID, Item_ID, Color_ID, Size_ID, BOM_CONS, DYE_MODE  FRom (Select RowID, PlanDtlID, OrdeR_NO, OrdeR_ID, ItemID Item_ID, COLORID Color_ID, SIZEID Size_ID, CAst((BOM_CONS + LOSS_WEIGHT) as Numeric(30,3)) BOM_CONS, DYE_MODE From FitSocks.Dbo.Socks_Yarn_Planning_Fn () Union all  Select RowID, PlanDtlID, Order_No, OrdeR_ID, ItemID ITEM_ID, DYE_ITEMID COLOR_ID , SIZEID SIZE_ID, CAst((BOM_CONS + LOSS_WEIGHT) as Numeric(30,3)) BOM_CONS, 'N' DYE_MODE From FitSocks.Dbo.Socks_Yarn_Planning_Fn () Where DYE_MODE = 'Y' )A  Where Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and Item_ID = " + Grid_Budget["ItemID", i].Value.ToString() + " and Color_ID = " + Grid_Budget["ColorID", i].Value.ToString() + " and Size_ID = " + Grid_Budget["SizeID", i].Value.ToString() + " ";
                                Queries[Array_Index++] = "Update FitSocks.Dbo.Socks_Planning_Yarn_Details Set Approval_Flag = 'T' , Approval_Time = Getdate(), Approval_System = Host_Name() Where RoWID In (Select Distinct PlanDtlID  From Socks_Yarn_Planning_Fn() Where Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and ItemID = " + Grid_Budget["ItemID", i].Value.ToString() + " and ColorID = " + Grid_Budget["ColorID", i].Value.ToString() + " and SizeID = " + Grid_Budget["SizeID", i].Value.ToString() + ") ";
                            ...
                            {
                                Queries[Array_Index++] = "Delete From FitSocks.Dbo.Socks_Yarn_BOM_Status  Where Planning_Detail_ID In (Select Distinct PlanDtlID  From Socks_Yarn_Planning_Fn() Where Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and ItemID = " + Grid_Budget["ItemID", i].Value.ToString() + " and ColorID = " + Grid_Budget["ColorID", i].Value.ToString() + " and SizeID = " + Grid_Budget["SizeID", i].Value.ToString() + ")";
                                Queries[Array_Index++] = "Update FitSocks.Dbo.Socks_Planning_Yarn_Details Set Approval_Flag = 'F' , Approval_Time = Null, Approval_System = Null  Where RowID In (Select Distinct PlanDtlID  From Socks_Yarn_Planning_Fn() Where  Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and ItemID = " + Grid_Budget["ItemID", i].Value.ToString() + " and ColorID = " + Grid_Budget["ColorID", i].Value.ToString() + " and SizeID = " + Grid_Budget["SizeID", i].Value.ToString() + ") ";                              
                        ...
                        else if (Convert.ToInt32(Dt_Budget.Rows[i]["O_Slno"]) == 2)
                        {
                            if (Grid_Budget[k, i].Style.BackColor == Color.Green)
                            {
                                Queries[Array_Index++] = "Insert Into FitSocks.Dbo.Socks_Trims_BOM_Status (Planning_Master_ID, Planning_Detail_ID, Order_ID, Item_ID, Color_ID, Size_ID, BOM, Dyeing_Status)  Select RowID, TrimDtlID,  OrdeR_ID, Item_ID, COLOR_ID, SIZE_ID, Tot_Qty , 'N' DYE_MODE From FitSocks.Dbo.Socks_Trim_Planning_Fn() Where  Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and Item_ID = " + Grid_Budget["ItemID", i].Value.ToString() + " and Color_ID = " + Grid_Budget["ColorID", i].Value.ToString() + " and Size_ID = " + Grid_Budget["SizeID", i].Value.ToString() + " ";
                                Queries[Array_Index++] = "Update FitSocks.Dbo.Socks_Planning_Trim_Details Set Approval_Flag = 'T' , Approval_Time = Getdate(), Approval_System = Host_Name() Where RoWID In (Select Distinct TrimDtlID  From Socks_Trim_Planning_Fn() Where  Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and Item_ID = " + Grid_Budget["ItemID", i].Value.ToString() + " and Color_ID = " + Grid_Budget["ColorID", i].Value.ToString() + " and Size_ID = " + Grid_Budget["SizeID", i].Value.ToString() + ") ";
                                Queries[Array_Index++] = "Insert into Vaahini_ERP_Gainup.Dbo.Budget_Approval_Status (Order_No, ProcessID, ItemID, ColorID, SizeID, Status) Values ('" + Grid_Budget.Columns[k].Name.ToString() + "', " + Grid_Budget["ProcessID", i].Value.ToString() + ", " + Grid_Budget["ItemID", i].Value.ToString() + ", " + Grid_Budget["ColorID", i].Value.ToString() + ", " + Grid_Budget["SizeID", i].Value.ToString() + ", 'T')";
                            }
                            else if(Grid_Budget[k, i].Style.BackColor != Color.Yellow)                            
                            {
                                Queries[Array_Index++] = "Delete From FitSocks.Dbo.Socks_Trims_BOM_Status  Where Planning_Detail_ID  In (Select Distinct TrimDtlID  From Socks_Trim_Planning_Fn() Where Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and Item_ID = " + Grid_Budget["ItemID", i].Value.ToString() + " and Color_ID = " + Grid_Budget["ColorID", i].Value.ToString() + " and Size_ID = " + Grid_Budget["SizeID", i].Value.ToString() + ") ";
                                Queries[Array_Index++] = "Update FitSocks.Dbo.Socks_Planning_Trim_Details Set Approval_Flag = 'F' , Approval_Time = Null, Approval_System = Null   Where RoWID In (Select Distinct TrimDtlID  From Socks_Trim_Planning_Fn() Where  Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and Item_ID = " + Grid_Budget["ItemID", i].Value.ToString() + " and Color_ID = " + Grid_Budget["ColorID", i].Value.ToString() + " and Size_ID = " + Grid_Budget["SizeID", i].Value.ToString() + ") ";
                                Queries[Array_Index++] = "Insert into Vaahini_ERP_Gainup.Dbo.Budget_Approval_Status (Order_No, ProcessID, ItemID, ColorID, SizeID, Status) Values ('" + Grid_Budget.Columns[k].Name.ToString() + "', " + Grid_Budget["ProcessID", i].Value.ToString() + ", " + Grid_Budget["ItemID", i].Value.ToString() + ", " + Grid_Budget["ColorID", i].Value.ToString() + ", " + Grid_Budget["SizeID", i].Value.ToString() + ", 'N')";
                            }
                        }
                        else if (Convert.ToInt32(Dt_Budget.Rows[i]["O_Slno"]) == 4)
                        {
                            if(Dt_Budget.Rows[i]["ProcessID"].ToString() != "158")
                            {
                                if (Grid_Budget[k, i].Style.BackColor == Color.Green )
                                {
                                    Queries[Array_Index++] = "Update FitSocks.Dbo.Socks_Planning_Proc_Details Set Approval_Flag = 'T' , Approval_Time = Getdate(), Approval_System = Host_Name() Where  RoWID In (Select Distinct PlanDtlID  From Socks_Process_Planning_Fn() Where Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and Proc_ID = " + Grid_Budget["ProcessID", i].Value.ToString() + " and Sample_ID = " + Grid_Budget["ColorID", i].Value.ToString() + " and SizeID = " + Grid_Budget["SizeID", i].Value.ToString() + ") ";
                                    Queries[Array_Index++] = "Insert into Vaahini_ERP_Gainup.Dbo.Budget_Approval_Status (Order_No, ProcessID, ItemID, ColorID, SizeID, Status) Values ('" + Grid_Budget.Columns[k].Name.ToString() + "', " + Grid_Budget["ProcessID", i].Value.ToString() + ", " + Grid_Budget["ItemID", i].Value.ToString() + ", " + Grid_Budget["ColorID", i].Value.ToString() + ", " + Grid_Budget["SizeID", i].Value.ToString() + ", 'T')";
                                }
                                else if(Grid_Budget[k, i].Style.BackColor != Color.Yellow)                            
                                {
                                    Queries[Array_Index++] = "Update FitSocks.Dbo.Socks_Planning_Proc_Details Set Approval_Flag = 'F' , Approval_Time = Null, Approval_System = Null  Where  RoWID In (Select Distinct PlanDtlID  From Socks_Process_Planning_Fn() Where Order_No = '" + Grid_Budget.Columns[k].Name.ToString() + "' and Proc_ID = " + Grid_Budget["ProcessID", i].Value.ToString() + " and Sample_ID = " + Grid_Budget["ColorID", i].Value.ToString() + " and SizeID = " + Grid_Budget["SizeID", i].Value.ToString() + ") ";
                                    Queries[Array_Index++] = "Insert into Vaahini_ERP_Gainup.Dbo.Budget_Approval_Status (Order_No, ProcessID, ItemID, ColorID, SizeID, Status) Values ('" + Grid_Budget.Columns[k].Name.ToString() + "', " + Grid_Budget["ProcessID", i].Value.ToString() + ", " + Grid_Budget["ItemID", i].Value.ToString() + ", " + Grid_Budget["ColorID", i].Value.ToString() + ", " + Grid_Budget["SizeID", i].Value.ToString() + ", 'N')";
                                }
                            }
                        }
