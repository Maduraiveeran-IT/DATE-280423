...
namespace Accounts
{
    public partial class FrmFloorPairing : Form, Entry 
    {
        ...
        public FrmFloorPairing()
        {
            InitializeComponent();
        }

        private void FrmFloorPairing_Load(object sender, EventArgs e)
        {
            try
            {
                MyParent = (MDIMain)this.MdiParent;
                Buffer_Table = "Pair_" + Environment.MachineName.Replace("-", "") + "_" + MyParent.UserCode.ToString();
                MyBase.Clear(this);
            }
            catch (Exception ex)
            {
        ...
        public void Entry_Edit()
        {
            try
            {
                ...
                Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Pairing - Edit", "Select F1.EntryNo, F1.ENtryDate, S1.Shiftcode2 Shift, F1.Timing, F1.ShiftCode, F1.Remarks, F1.RowID From Floor_Pairing_Master F1 Left Join Socks_Shift () S1 on F1.ShiftCode = S1.Shiftcode Where F1.ENtryDate >='14-nov-2015' and F1.ENtryDate >= Dateadd (D, " + MyParent.User_Datelock + ", Cast(GetDate() as Date))", String.Empty, 80, 90, 70);
                if (Dr != null)
                {
                    Fill_Datas(Dr);
                }
        ...
        public void Entry_Save()
        {
            ...
            try
            {
                ...
                TxtNo.Text = MyBase.MaxOnlyComp("Floor_Pairing_Master", "EntryNo", String.Empty, MyParent.YearCode, MyParent.CompCode).ToString();

                if (MyParent._New)
                {
                    Queries[Array_Index++] = "Insert into Floor_Pairing_master (EntryNo, EntryDate, ShiftCode, Timing, Company_Code, EntryTime, EntrySystem, Remarks) Values (" + TxtNo.Text + ",'" + String.Format("{0:dd-MMM-yyyy}", DtpDate1.Value) + "','" + TxtShift.Tag.ToString() + "','" + TxtTiming.Text.ToString() + "'," + MyParent.CompCode + ",getdate(),Host_name(), '" + TxtRemarks.Text + "') ; Select Scope_Identity()";
                }
                else
                {
                    Queries[Array_Index++] = "Update Floor_Pairing_Master Set ShiftCode = " + TxtShift.Tag.ToString() + ", Remarks = '" + TxtRemarks.Text + "' where RowID = " + Code;
                    Queries[Array_Index++] = "Update F1 Set F1.PairQty = F1.PairQty - Isnull(F2.Production, 0) From Floor_Stock F1 Inner join (Select Order_No, OrderColorID, SizeID, ItemID, Sum(Production) Production From Floor_Pairing_Details Where MasterID = " + Code + " Group By Order_No, OrderColorID, SizeID, ItemID) F2 on F1.Order_No = F2.Order_No and F1.OrderColorID = F2.OrderColorID and F1.SizeID = F2.SizeID and F1.ItemID = F2.ItemID";
                    Queries[Array_Index++] = "Delete From Floor_Pairing_Details where MasterID = " + Code;
                }
                ...
                for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                {
                    ...
                    {
                        Queries[Array_Index++] = "Insert Into Floor_Pairing_Details (MasterID, Tag_No, Order_No, OrderColorID, ItemID, SizeID, BOMQty, production, Rework) Values (@@IDENTITY, '" + Grid["Tag_No", i].Value.ToString() + "',  '" + Grid["Order_No", i].Value.ToString() + "', " + Grid["OrderColorID", i].Value.ToString() + ", " + Grid["ItemID", i].Value.ToString() + ", " + Grid["SizeID", i].Value.ToString() + ", " + Grid["Bom", i].Value + ", " + Grid["Production", i].Value + ", " + Grid["Rework", i].Value + ")";
                    }
                    else
                    {
                        Queries[Array_Index++] = "Insert Into Floor_Pairing_Details (MasterID, Tag_No, Order_No, OrderColorID, ItemID, SizeID, BOMQty, production, Rework) Values (" + Code + ", '" + Grid["Tag_No", i].Value.ToString() + "', '" + Grid["Order_No", i].Value.ToString() + "', " + Grid["OrderColorID", i].Value.ToString() + ", " + Grid["ItemID", i].Value.ToString() + ", " + Grid["SizeID", i].Value.ToString() + ", " + Grid["Bom", i].Value + ", " + Grid["Production", i].Value + ", " + Grid["Rework", i].Value + ")";
                    }

                    Queries[Array_Index++] = "Update Floor_Stock set Act_PairQty = Act_PairQty + " + Grid["Production", i].Value + " where Order_No = '" + Grid["Order_No", i].Value.ToString() + "' and OrderColorID = " + Grid["OrderColorID", i].Value + " and SizeID = " + Grid["SizeID", i].Value.ToString() + " and ItemID = '" + Grid["ItemID", i].Value.ToString() + "'";
                    Queries[Array_Index++] = "Update Floor_Stock set PairQty = PairQty + " + Grid["Production", i].Value + " where Order_No = '" + Grid["Order_No", i].Value.ToString() + "' and OrderColorID = " + Grid["OrderColorID", i].Value + " and SizeID = " + Grid["SizeID", i].Value.ToString() + " and ItemID = '" + Grid["ItemID", i].Value.ToString() + "'";
                }
        ...
        public void Entry_Delete()
        {
            try
            {
                ...
                Set_Min_Max_Date(true);
                Buffer_Update = false;
                Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Pairing - Delete", "Select F1.EntryNo, F1.ENtryDate, S1.Shiftcode2 Shift, F1.Timing, F1.ShiftCode, F1.Remarks, F1.RowID From Floor_Pairing_Master F1 Left Join Socks_Shift () S1 on F1.ShiftCode = S1.Shiftcode where F1.ENtryDate >='14-nov-2015' ", String.Empty, 80, 90, 70);
                if (Dr != null)
                {
                    Fill_Datas(Dr);
        ...
        public void Entry_Delete_Confirm()
        {
            try
            {
                if (Code > 0)
                {
                    MyBase.Run("Update F1 Set F1.PairQty = F1.PairQty - Isnull(F2.Production, 0), F1.Act_PairQty = F1.Act_PairQty - Isnull(F2.Production, 0) From Floor_Stock F1 Inner join (Select Order_No, OrderColorID, ItemID, SizeID, Sum(Production) Production From Floor_Pairing_Details Where MasterID = " + Code + " Group By Order_No, OrderColorID, SizeID, ItemID) F2 on F1.Order_No = F2.Order_No and F1.OrderColorID = F2.OrderColorID and F1.SizeID = F2.SizeID", "Delete From Floor_Pairing_Details where MasterID = " + Code, "Delete From Floor_Pairing_Master where RowID = " + Code);
        ...
        public void Entry_View()
        {
            try
            {
                ...
                Buffer_Update = false;
                Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Pairing - View", "Select F1.EntryNo, F1.ENtryDate, S1.Shiftcode2 Shift, F1.Timing, F1.ShiftCode, F1.Remarks, F1.RowID From Floor_Pairing_Master F1 Left Join Socks_Shift () S1 on F1.ShiftCode = S1.Shiftcode", String.Empty, 80, 90, 70);
                if (Dr != null)
                {
                    Fill_Datas(Dr);
                }
        ...
        void Grid_Data(Boolean Buffer)
        {
            ...
            try
            {
                if (Buffer)
                {
                    Str = "Select 0 as Slno, F1.Tag_No, F1.Order_No, F1.OrderColorID, Sample, F1.ItemID, Item, F1.SizeID, S1.Size, F1.BOMQty Bom, F1.SettedQty, F1.Production, F1.Rework, '-' T From " + Buffer_Table + " F1 Left Join size S1 on F1.SizeID = S1.sizeid Order By F1.Slno";
                }
                else
                {
                    if (MyParent._New)
                    {
                        Str = "Select 0 as Slno,F1.Tag_No, F1.Order_No, F1.OrderColorID, Cast('' As Varchar (15)) Sample, F1.ItemID, Cast('' As Varchar (15)) Item, F1.SizeID, Cast('' As Varchar (15)) Size, F1.BOMQty Bom, Cast('' As numeric(25,2))SettedQty, F1.Production, F1.Rework, '-' T From Floor_Pairing_Details F1 Left Join size S1 on F1.SizeID = S1.sizeid Left Join Vaahini_ERP_Gainup.Dbo.Employeemas E1 on F1.Emplno_Operator = E1.Emplno Where 1 = 2";
                    }
                    else
                    {
                        Str = "Select 0 as Slno,F1.Tag_No, F1.Order_No, F1.OrderColorID, C1.color Sample, F1.ItemID, C1.Item, F1.SizeID, S1.Size, F1.BOMQty Bom, F2.SetQty SettedQty, F1.Production, F1.Rework, '-' T From Floor_Pairing_Details F1 Left Join Socks_Bom() C1 On F1.OrderColorID = C1.OrderColorId And F1.Order_No = C1.Order_No And F1.ItemID = C1.ItemID And F1.SizeID = C1.SizeID Left Join Floor_Stock F2 On F1.Order_No = F2.Order_No and F1.OrderColorID = F2.OrderColorID and F1.SizeID = F2.SizeID Left Join size S1 on F1.SizeID = S1.sizeid Left Join Vaahini_ERP_Gainup.Dbo.Employeemas E1 on F1.Emplno_Operator = E1.Emplno Where F1.MasterID = " + Code + " Order By F1.RowID ";
                    }
                }

                Grid.DataSource = MyBase.Load_Data(Str, ref Dt);
                MyBase.Grid_Designing(ref Grid, ref Dt, "OrderColorID", "SizeID", "ItemID", "T");
                MyBase.ReadOnly_Grid_Without(ref Grid, "Tag_No", "Production", "Rework");
                MyBase.Grid_Width(ref Grid, 50, 100, 120, 120, 100, 100, 100, 100, 100);
        ...
        private void FrmFloorPairing_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void FrmFloorPairing_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
        ...
        void Txt_GotFocus(object sender, EventArgs e)
        {
            try
            {
                ...
                //}
                //else 
                if (Grid.CurrentCell.ColumnIndex == Grid.Columns["Tag_No"].Index && Grid.CurrentCell.Value.ToString() == String.Empty)
                {
                    if (Grid.CurrentCell.RowIndex > 0)
                    {
                        Total_Prod_Qty();
        ...
        void Total_Prod_Qty()
        {
            try
            {
                TxtTotal.Text = String.Format("{0:0.0}", Convert.ToDouble(MyBase.Sum(ref Grid, "Production", "Order_No", "Sample")));
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        void OrderNo_Selection()
        {
            try
            {
                Dr = Tool.Selection_Tool(this, 150, 150, SelectionTool_Class.ViewType.NormalView, "Select Tag No", "select Tag_No, Order_No, Item, Sample, Size, BOMQty BOM, Setted_Qty, ItemID, OrderColorID, SizeID from Floor_Pairing_Input_New() Where Setted_Qty > 0", String.Empty, 100, 120, 100, 100, 100, 100, 100);
                if (Dr != null)
                {
                    ...
                    Grid["SettedQty", Grid.CurrentCell.RowIndex].Value = Dr["Setted_Qty"].ToString();
        ...
        void Operator_Selection()
        {
            try
            {
                Dr = Tool.Selection_Tool(this, 150, 150, SelectionTool_Class.ViewType.NormalView, "Select Operator", "Select Name, Tno, Emplno From Socks_Employee_Present_Detail ('" + String.Format("{0:dd-MMM-yyyy}", DtpDate1.Value) + "') Where DeptName = 'PACKING' and  Tno Not Like '%Z'", String.Empty, 250, 80);
                if (Dr != null)
                {
        ...
        void Fill_BOM(String OrderNo, String Sample, String Size)
        {
            ...
            {
                DataTable Tdt = new DataTable();
                MyBase.Load_Data("Select S1.Bom_Qty Bom, cast(isnull(F1.SetQty,0) as numeric(25,2)) Setted, cast(Isnull(K1.Paired, 0) as numeric(25,2)) Paired, cast((isnull(F1.SetQty,0) - Isnull(K1.Paired, 0)) as numeric(25,2)) Balance_Pairing From Socks_Bom () S1 Left Join Pairing_Production_All () k1 on S1.Order_No = K1.OrderNo and S1.OrderColorId = K1.OrderColorID and S1.sizeid = K1.SizeID Left Join Floor_Stock F1 On S1.Order_No = F1.Order_No and S1.ItemID = F1.ItemID and S1.OrderColorID = F1.OrderColorID and S1.sizeid = F1.SizeID Where S1.Order_No = '" + OrderNo + "' And S1.color = '" + Sample + "' and S1.Size = '" + Size + "'", ref Tdt);
                if (Tdt.Rows.Count > 0)
                {
                    LblSetted.Text = "SETTED: " + Tdt.Rows[0]["Setted"].ToString();
                    LblPre_Prod.Text = "PROD: " + Tdt.Rows[0]["Paired"].ToString();
                    LblBal.Text = "BAL: " + Tdt.Rows[0]["Balance_Pairing"].ToString();
                ...
                {
                    Tdt = new DataTable();
                    MyBase.Load_Data("Select Isnull(Sum(Production), 0) Production From Floor_Pairing_Details Where Order_No = '" + OrderNo + "' And OrderColorID = .Dbo.Get_OrdercolorID ('" + OrderNo + "', '" + Sample + "') and SizeID = Dbo.Get_OrderSizeID ('" + OrderNo + "', '" + Size + "') and MasterID = " + Code, ref Tdt);
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Buffer_Update)
                {
                    ...
                    {
                        MyBase.Execute("Select Cast(0 as int) Slno, Tag_No, Order_No, OrderColorID, cast('' as varchar(20)) Sample, ItemID, cast('' as varchar(20)) Item, SizeID, cast('' as varchar(20)) Size, BomQty, Cast(0 as numeric(25,2)) SettedQty, Production, Rework into " + Buffer_Table + " From Floor_Pairing_Details Where 1 = 2");
                    }
                    ...
                    for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                    {
                        Queries[Array_Index++] = "Insert Into " + Buffer_Table + " (Slno, Tag_No, Order_No, OrderColorID, Sample, ItemID, Item, SizeID, Size, BOMQty, SettedQty, production, Rework) Values (" + Grid["Slno", i].Value.ToString() + ", '" + Grid["Tag_No", i].Value.ToString() + "', '" + Grid["Order_No", i].Value.ToString() + "', " + Grid["OrderColorID", i].Value.ToString() + ", '" + Grid["Sample", i].Value.ToString() + "', " + Grid["ItemID", i].Value.ToString() + ", '" + Grid["Item", i].Value.ToString() + "', " + Grid["SizeID", i].Value + ", '" + Grid["Size", i].Value.ToString() + "', " + Grid["Bom", i].Value + ", " + Grid["SettedQty", i].Value + ", " + Grid["Production", i].Value + ", " + Grid["Rework", i].Value + ")";
                    }
        ...
        private void LblLinked_Click(object sender, EventArgs e)
        {
        ...
        private void LblSetted_Click(object sender, EventArgs e)
        {

        }

        private void LblPre_Prod_Click(object sender, EventArgs e)
        {
