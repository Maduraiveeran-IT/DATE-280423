...
namespace Accounts
{
    public partial class FrmSocksGeneralGRN : Form, Entry
    {
        MDIMain MyParent;
        ...
        public FrmSocksGeneralGRN()
        {
            InitializeComponent();
        }

        private void FrmSocksGeneralGRN_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        void Load_OCN_Lot_EDIT()
        {
            try
            {
                for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                {
                    ...
                    MyBase.Load_Data("Select S1.Slno SL, (S2.Order_No + '-' + S8.PONo) Description, S1.RowID OCN_RowID, S1.Order_ID, S1.PO_Detail_ID, S2.Order_No, S8.PONo, (S9.Bal_Qty + S1.Qty) PO_QTY, S1.Qty GRN_QTY, '' T From Socks_General_GRN_OCN_DEtails S1 left Join VSocks_Sample_Req_Master  S2 on S1.Order_ID = S2.RowID Inner Join Socks_General_PO_Details S7 on S1.PO_Detail_ID = S7.RowID Inner Join Socks_General_PO_Master S8 on S7.Master_ID = S8.RowID Left join Socks_General_GRN_Pending_OCN() S9 on S7.Item_id = S9.Item_id and S7.Color_id = S9.Color_id and S7.Size_ID = S9.Size_ID and S1.Order_ID = S9.Order_ID and S9.PO_Detail_ID = S7.RowID Where S1.Detail_ID = " + Dt.Rows[i]["Detail_ID"].ToString(), ref Dt_OCN_New[i, 0, 0]);

                    for (int j = 0; j <= Dt_OCN_New[i, 0, 0].Rows.Count - 1; j++)
                    {
                        Dt_OCN_New[i, j, 1] = new DataTable();
                        MyBase.Load_Data("Select S1.Slno SL, S1.Lot_No, S1.Bag_No, S1.Qty, S1.Location_ID, S3.Location, '' T From Socks_General_GRN_OCN_Lot_Details S1 Left Join Socks_Yarn_Stores_location_Master S3 on S1.Location_ID = S3.rowID Where S1.OCN_RowID = " + Dt_OCN_New [i, 0, 0].Rows[j]["OCN_RowID"].ToString() , ref Dt_OCN_New[i, j, 1]);
                    }
                }
            }
            catch (Exception ex)
        ...
        void GRN_Generate()
        {
            try
            {
                if(MyParent._New)
                {
                    ...
                    MyBase.Load_Data("Select DBo.Get_Max_Socks_General_GRN ('" + String.Format("{0:dd-MMM-yyyy}", DtpDate.Value) + "')", ref Tdt);
        ...
        public void Entry_Save()
        {
            ...
            try
            {
                ...
                for (int i=0;i<=Dt.Rows.Count - 1;i++)
                {
                    MyBase.SQLCmd.CommandText = "Insert Into Socks_General_GRN_Details (Master_ID, SlNo, Item_ID, Color_ID, Size_ID, Grn_Qty, Rate, Amount) Values (" + Code + ", " + (i + 1) + ", " + Grid["Item_ID", i].Value.ToString() + ", " + Grid["Color_ID", i].Value.ToString() + ", " + Grid["Size_ID", i].Value.ToString() + ", " + Grid["GRN_Qty", i].Value.ToString() + ", " + Grid["Rate", i].Value.ToString() + ", " + Grid["Amount", i].Value.ToString() + "); Select Scope_Identity()";
                    Detail_ID = Convert.ToInt64(MyBase.SQLCmd.ExecuteScalar());

                    Slno = 0;
                    for (int j=0;j<= Dt_OCN_New[i, 0, 0].Rows.Count - 1;j++)
                    {
                         if(Dt_OCN_New[i, 0, 0].Rows[j]["Description"].ToString() == "GUP-GNO00000-GUP-POG00000")
                        {
                            MyBase.SQLCmd.CommandText = " Exec  Socks_General_FreeOcn_Item_Proc " + Grid["Item_ID", i].Value.ToString() + ", " + Grid["Color_ID", i].Value.ToString() + ", " + Grid["Size_ID", i].Value.ToString() + ", " + Dt_OCN_New[i, 0, 0].Rows[j]["GRN_Qty"].ToString() + " ";
                            MyBase.SQLCmd.ExecuteNonQuery();
                            ...
                            if (Convert.ToInt64(Dt_OCN_New[i, 0, 0].Rows[j]["PO_Detail_ID"].ToString()) == 0)
                            {
                                ...
                                MyBase.Load_Data("Select Ident_Current('Socks_General_PO_Details')", ref TDtPoID);
                            ...
                            MyBase.SQLCmd.CommandText = "Insert Into Socks_General_GRN_OCN_Details (Master_ID, Slno, PO_Detail_ID, Order_ID, Qty, Detail_ID, Trans_Mode) Values (" + Master_ID + ", " + Slno + ", " + PoDtl_ID + ", " + Dt_OCN_New[i, 0, 0].Rows[j]["Order_ID"].ToString() + ", " + Dt_OCN_New[i, 0, 0].Rows[j]["GRN_Qty"].ToString() + ", " + Detail_ID + ", '" + Dt_OCN_New[i, 0, 0].Rows[j]["T"].ToString() + "'); Select Scope_Identity() ";
                        ...
                        {
                            MyBase.SQLCmd.CommandText = "Insert Into Socks_General_GRN_OCN_Details (Master_ID, Slno, PO_Detail_ID, Order_ID, Qty, Detail_ID, Trans_Mode) Values (" + Master_ID + ", " + Slno + ", " + Dt_OCN_New[i, 0, 0].Rows[j]["PO_Detail_ID"].ToString() + ", " + Dt_OCN_New[i, 0, 0].Rows[j]["Order_ID"].ToString() + ", " + Dt_OCN_New[i, 0, 0].Rows[j]["GRN_Qty"].ToString() + ", " + Detail_ID + ", '" + Dt_OCN_New[i, 0, 0].Rows[j]["T"].ToString() + "'); Select Scope_Identity() ";
                             OCN_ID = Convert.ToInt64(MyBase.SQLCmd.ExecuteScalar());                                                                                              
                         }
                        for (int k = 0; k <= Dt_OCN_New[i, j, 1].Rows.Count - 1; k++)
                        {
                            MyBase.SQLCmd.CommandText = "Insert Into Socks_General_GRN_OCN_Lot_Details (Master_ID, OCN_RowID, Slno, Lot_No, Bag_No, Location_ID, Qty, Supplier_Code) Values (" + Master_ID + ", " + OCN_ID + ", " + (k + 1) + ", '" + Dt_OCN_New[i, j, 1].Rows[k]["Lot_No"].ToString() + "', " + Dt_OCN_New[i, j, 1].Rows[k]["Bag_No"].ToString() + ", " + Dt_OCN_New[i, j, 1].Rows[k]["Location_ID"].ToString() + ", " + Dt_OCN_New[i, j, 1].Rows[k]["Qty"].ToString() + ", " + TxtSupplier.Tag.ToString() + ")";
                            MyBase.SQLCmd.ExecuteNonQuery();
                        }
                    }
                }
                ...
                for (int i = 0; i <= Dt_Tax.Rows.Count - 1; i++)
                {
                    if (MyParent._New)
                    {
                        MyBase.SQLCmd.CommandText = "Insert Into Socks_General_GRN_Tax_Details (Master_ID, Slno, Tax_Code, Mode, Tax_Per, Tax_Amount) Values (" + Code + ", " + (i + 1) + ", " + Dt_Tax.Rows[i]["Tax_Code"].ToString() + ", '" + Dt_Tax.Rows[i]["Tax_Mode"].ToString() + "', " + Dt_Tax.Rows[i]["Tax_Per"].ToString() + ", " + Dt_Tax.Rows[i]["Tax_Amount"].ToString() + ")";
                        MyBase.SQLCmd.ExecuteNonQuery();
                    }
                    else
                    {
                ...
                if (Grid["Item_Type", 0].Value.ToString() == "YARN")
                {
                    if (MyParent.Edit)
                    {
                        MyBase.SQLCmd.CommandText = " Exec Vsocks_General_lot_Delete " + Code + " ";
                        MyBase.SQLCmd.ExecuteNonQuery();
                    }
                    MyBase.SQLCmd.CommandText = " Exec VSocks_General_Lot_Insert " + Master_ID + " ";
                    MyBase.SQLCmd.ExecuteNonQuery();
                }
                else
                {
                    if (MyParent.Edit)
                    {
                        MyBase.SQLCmd.CommandText = " Exec Vsocks_General_TrimsGrn_Stock_Delete  " + Code + " ";
                        MyBase.SQLCmd.ExecuteNonQuery();
                    }
                    MyBase.SQLCmd.CommandText = " Exec Vsocks_General_TrimsGrn_Stock_Insert " + Master_ID + " ";
                    MyBase.SQLCmd.ExecuteNonQuery();
                }
        ...
        public void Entry_Delete_Confirm()
        {
            try
            {
                if (Code > 0)
                {
                    ...
                    MyBase.SQLCmd.CommandText = "Delete From Socks_General_GRN_Details Where Master_ID = " + Code;
                    MyBase.SQLCmd.ExecuteNonQuery();
                    MyBase.SQLCmd.CommandText = "Delete From Socks_General_Grn_Master Where RowID = " + Code;
                    MyBase.SQLCmd.ExecuteNonQuery();
                    ...
                    MyBase.SQLCmd.CommandText = MyParent.EntryLog("SOCKS GENERAL GRN", "DELETE", Code.ToString());
                    MyBase.SQLCmd.ExecuteNonQuery();
        ...
        public void Entry_Print()
        {
            try
            {
                ...
                Str1 = " Select Top 10000000 B.Item+' - '+C.Color+' - '+D.Size Particulars, A.Grn_Qty, A.Rate, A.Amount From Socks_General_GRN_Details A Left Join Item B On A.Item_ID = B.ItemID Left Join Color C On A.Color_ID = C.ColorID Left Join Size D On A.Size_ID = D.SizeID Where A.Master_ID = " + Code + " Order By B.Item+' - '+C.Color+' - '+D.Size ";
                MyBase.Execute_Qry(Str1, "Yarn_Goods_Receipt");

                Str2 = " Select Dbo.General_PoWise_Receipt_Qty(" + Code + ")PoDetails";
                ...
                MyParent.CReport(ref ObjRpt, "General Purchase Goods Receipt..!");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        void Load_Item()
        {
            try
            {
                ...
                if (MyParent._New)
                {
                    Grid.DataSource = MyBase.Load_Data("Select 0 as SL, Cast(0 as Bigint) Detail_ID, Item + ' ' + Color + ' ' + Size + ' @ ' + Cast(Rate as Varchar (15)) Description, Item_ID, Item ITEM, Color_ID, Color COLOR, Size_ID, Size SIZE, PO_Qty, Inward_Qty, Bal_Qty, Bal_Qty GRN_Qty, Rate, Cast(0 as Numeric (25, 2)) Amount, ITem_Type, 0.000 Limit From Socks_General_GRN_Pending () Where 1=2", ref Dt);
                }
                else
                {
                    Grid.DataSource = MyBase.Load_Data("Select S2.Slno SL, S2.RowID Detail_ID, S1.Item + ' ' + S1.Color + ' ' + S1.Size + ' @ ' + Cast(S1.Rate as Varchar (15)) Description, S1.Item_ID, S1.Item ITEM, S1.Color_ID, S1.Color COLOR, S1.Size_ID, S1.Size SIZE, S1.PO_Qty, (S1.Inward_Qty) Inward_Qty, (S1.Bal_Qty + S2.GRN_Qty) Bal_Qty, S2.GRN_Qty, S2.Rate, (S2.Rate * S2.GRN_Qty) Amount, S1.ITem_Type, (S1.Bal_Qty + S2.GRN_Qty) + (Case When 15 < Cast(Nullif(((S1.Bal_Qty + S2.GRN_Qty) * 5),0) /100 as Numeric(20,3)) Then 15 Else Cast(Nullif(((S1.Bal_Qty + S2.GRN_Qty) * 5),0) /100 as Numeric(20,3)) End) Limit From Socks_General_GRN_Pending () S1 Inner join Socks_General_Grn_Details S2 on S1.Item_id = S2.Item_ID and S1.Color_id = S2.Color_ID and S1.Size_ID = S2.Size_ID And S1.Rate = S2.Rate Inner Join Socks_General_GRN_Master S3 On S2.Master_ID = S3.RowID and S1.Supplier_Code = S3.Supplier_Code  Where S2.Master_ID = " + Code + " Order By S2.Slno", ref Dt);
                }
                MyBase.Grid_Designing(ref Grid, ref Dt, "Item_ID", "Color_ID", "Detail_ID", "Size_ID", "Description", "Item_Type");
        ...
        private void FrmSocksGeneralGRN_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    if (this.ActiveControl.Name == "TxtSupplier")
                    {
                        ...
                        { 
                            Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select Supplier", "Select Distinct L1.Ledger_Name Supplier, S1.SUpplier_Code From Socks_General_GRN_Pending () S1 Left Join Supplier_All_Fn() L1 on S1.Supplier_Code = L1.Ledger_Code ", String.Empty, 350);
                            if (Dr != null)
                            {
                                DataTable Dts = new DataTable();
                        ...
                        if (this.ActiveControl.Name == "TxtGatePass")
                        {
                            ...
                            Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select GATE PASS", "Select GPNo, GPDate, Party, isnull(InvNo, '') InvNo, InvDate, Isnull(DCno, '') DCno, DCDate, Qty From Socks_General_Gate_Pass_Details_Pending()", String.Empty, 100, 100, 150, 100, 100, 100, 100);
                            if (Dr != null)
                            {
        ...
        private void FrmSocksGeneralGRN_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                ...
                if (e.KeyCode == Keys.Down)
                {
                    if (Grid.CurrentCell.ColumnIndex == Grid.Columns["Item"].Index)
                    {
                        ...
                        if (Dr != null)
                        {
                            ...
                            Grid["Item_Type", Grid.CurrentCell.RowIndex].Value = Dr["Item_Type"].ToString();
                            Txt_Gross.Tag = Dr["Item_Type"].ToString();
        ...
        void Load_Lot(Int32 Row)
        {
            try
            {
                if (Dt_OCN_New[Grid.CurrentCell.RowIndex, Row, 1] == null)
                {
                    ...
                    MyBase.Load_Data("Select S1.Slno SL, S1.Lot_No, S1.Bag_No, S1.Qty, S1.Location_ID, S3.Location, '' T From Socks_General_GRN_OCN_Lot_Details S1 Left Join Socks_Yarn_Stores_location_Master S3 on S1.Location_ID = S3.rowID Where 1 = 2", ref Dt_OCN_New[Grid.CurrentCell.RowIndex, Row, 1]);
                }
        ...
        void Load_OCN(Int32 Row)
        {
            try
            {
                if (Dt_OCN_New[Row, 0, 0] == null)
                {
                    ...
                    MyBase.Load_Data("Select S1.Slno SL, Cast('' as Varchar (30)) Description, Cast(0 as Bigint) OCN_RowID, S1.Order_ID, S1.PO_Detail_ID, S2.Order_No, S8.PONo, Qty PO_QTY, Cast(0 as Numeric (16, 3)) GRN_QTY, 'N' T From Socks_General_GRN_OCN_DEtails S1 left Join VSocks_Sample_Req_Master S2 on S1.Order_ID = S2.RowID Inner Join Socks_General_PO_Details S7 on S1.PO_Detail_ID = S7.RowID Inner Join Socks_General_PO_Master S8 on S7.Master_ID = S8.RowID Where 1 = 2", ref Dt_OCN_New[Row, 0, 0]);
                }
        ...
        Int32 Get_Max_Bag_No(Int32 Supplier_Code, String Lot_No)
        {
            ...
            {
                DataTable Tdt = new DataTable();
                MyBase.Load_Data("Select Isnull(Max(Bag_No), 0) From Socks_General_GRN_Ocn_Lot_Details Where SUpplier_Code = " + Supplier_Code.ToString() + " and Lot_No = '" + Lot_No + "'", ref Tdt);
        ...
        void Load_Tax()
        {
            try
            {
                if (MyParent._New)
                {
                    Grid_Tax.DataSource = MyBase.Load_Data("Select S1.Slno Sl, S1.Tax_Code, L1.Ledger_Name Tax, S1.Mode Tax_Mode, S1.Tax_Per, S1.Tax_Amount, '' T From Socks_General_GRN_Tax_Details S1 Left Join Socks_Tax_Master L1 on S1.Tax_Code = L1.Ledger_Code  Where 1 = 2 Order by S1.Slno ", ref Dt_Tax);
                }
                else
                {
                    Grid_Tax.DataSource = MyBase.Load_Data("Select S1.Slno Sl, S1.Tax_Code, L1.Ledger_Name Tax, S1.Mode Tax_Mode, S1.Tax_Per, S1.Tax_Amount, '' T From Socks_General_GRN_Tax_Details S1 Left Join Socks_Tax_Accounts() L1 on S1.Tax_Code = L1.Ledger_Code  Where S1.Master_ID = " + Code + " Order by S1.Slno ", ref Dt_Tax);
                }
            ...
            try
            {
                ...
                if (e.KeyCode == Keys.Down)
                {
                    if (Grid_OCN.CurrentCell.ColumnIndex == Grid_OCN.Columns["ORDER_NO"].Index)
                    {
                        ...
                        //Dr = Tool.Selection_Tool_Except_New("Description", this, 30, 70, ref Dt_OCN_New[Grid.CurrentCell.RowIndex, 0, 0], SelectionTool_Class.ViewType.NormalView, "Select OCN", "Select A.OrdeR_No, A.PoNo, A.PO_Qty, A.Order_ID, IsNull(B.PO_Detail_ID, 0) PO_Detail_ID , A.OrdeR_No + '-' + A.PoNo Description From Socks_Sample_Yarn_Grn_Routine_Ocn_Fn(" + TxtSupplier.Tag + ", " + Grid["Item_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ", " + Grid["Color_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ", " + Grid["Size_ID", Grid.CurrentCell.RowIndex].Value.ToString() + ", " + Grid["Rate", Grid.CurrentCell.RowIndex].Value.ToString() + ", " + (Convert.ToDouble(Grid["GRN_Qty", Grid.CurrentCell.RowIndex].Value)) + ", " + (Convert.ToDouble(Grid["Bal_Qty", Grid.CurrentCell.RowIndex].Value)) + ") A LEft Join Socks_Sample_Yarn_GRN_Pending_OCN() B On A.PODtlID = B.Po_Detail_ID and A.Order_ID = B.ORder_ID  " , String.Empty, 120, 120, 100);
                        Dr = Tool.Selection_Tool_Except_New("Description", this, 30, 70, ref Dt_OCN_New[Grid.CurrentCell.RowIndex, 0, 0], SelectionTool_Class.ViewType.NormalView, "Select OCN", "Select OrdeR_No, PoNo, PO_Qty, Order_ID, PO_Detail_ID, Description  From (Select Distinct S2.Order_No, S1.PONo, S1.Bal_Qty PO_Qty, S1.Order_ID, S1.PO_Detail_ID, (S2.Order_No + '-' + S1.PONo) Description From Socks_General_GRN_Pending_OCN () S1 left join VSocks_Sample_Req_Master S2 on S1.Order_ID = S2.RowID Where S1.Bal_Qty > 0 and S1.Supplier_Code = " + TxtSupplier.Tag + " and Item_ID = " + Grid["Item_ID", Grid.CurrentCell.RowIndex].Value.ToString() + " and Color_ID = " + Grid["Color_ID", Grid.CurrentCell.RowIndex].Value.ToString() + " and Size_ID = " + Grid["Size_ID", Grid.CurrentCell.RowIndex].Value.ToString() + " and Rate = " + Grid["Rate", Grid.CurrentCell.RowIndex].Value.ToString() + "  and OrdeR_ID != 878 and S1.PODate <= '" + String.Format("{0:dd-MMM-yyyy}", DtpDate.Value) + "' Union all  Select Order_No, 'GUP-POG00000' PoNo, " + (Convert.ToDouble(Grid["GRN_Qty", Grid.CurrentCell.RowIndex].Value) - Convert.ToDouble(Grid["Bal_Qty", Grid.CurrentCell.RowIndex].Value)) + " Po_Qty, RowID OrdeR_ID, 0 Po_Detail_ID, Order_No + '-' + 'GUP-POG00000' Description From VSocks_Sample_Req_Master Where RowID = 878 and " + (Convert.ToDouble(Grid["GRN_Qty", Grid.CurrentCell.RowIndex].Value) - Convert.ToDouble(Grid["Bal_Qty", Grid.CurrentCell.RowIndex].Value)) + " > 0) A ORder by PONo ", String.Empty, 120, 120, 100, 200);
                        if (Dr != null)
                        {
                                ...
                                if (Dr["PONO"].ToString() == "GUP-POG00000" && Dr["ORDER_ID"].ToString() == "878")
                                {
                                    FStr = " Select Rowid FRom Socks_General_Po_Details Where ORdeR_ID =  878 and Item_ID = " + Grid["Item_ID", Grid.CurrentCell.RowIndex].Value.ToString() + " and Color_Id = " + Grid["Color_ID", Grid.CurrentCell.RowIndex].Value.ToString() + " and Size_ID = " + Grid["Size_ID", Grid.CurrentCell.RowIndex].Value.ToString()  + " ";
                                    MyBase.Load_Data(FStr, ref FDt);
                                    if (FDt.Rows.Count > 0)
                                    {
                                        Grid_OCN["PO_Detail_ID", Grid_OCN.CurrentCell.RowIndex].Value = FDt.Rows[0][0].ToString();
                                    }
                                    else
                                    {
                                        Grid_OCN["PO_Detail_ID", Grid_OCN.CurrentCell.RowIndex].Value = 0;
                                    }
                                }
                                else
                                {
