...
namespace Accounts
{
    public partial class Frm_Machine_Production_Import_Whole_Unit : Form
    {
        ...
        public Frm_Machine_Production_Import_Whole_Unit(DateTime FDate, DateTime TDate, Int16 FShift, Int16 TShift, Int32 FYear, Int32 TYear, Int32 FWeek, Int32 TWeek, Int32 Unit)
        {
            InitializeComponent();
            MyBase.Clear(this);
        ...
        private void Frm_Machine_Production_Import_Whole_Unit_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Frm_Machine_Production_Import_Whole_Unit_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        void Grid_Data()
        {
            try
            {
                Str = " Exec Import_Grid_Whole_Unit " + UnitCode_New + ", " + TxtFYear.Text + ", " + TxtFWeek.Text + ", '" + String.Format("{0:dd-MMM-yyyy}", DtpFDate.Value) + "', " + TxtFShift.Text + "";

                Grid.DataSource = MyBase.Load_Data(Str, ref Dt);
                MyBase.Grid_Designing(ref Grid, ref Dt, "Needle_ID", "Machine_ID", "OrderColorID", "Plan_Qty", "Emplno", "Planned_Seconds", "Qty", "Order_No2", "Prod_Qty", "T", "Mode", "Stored");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i <= Dt.Rows.Count - 1; i++)
                {
                    if (Dt.Rows[i]["Mode"].ToString() == "N" && Dt.Rows[i]["Machine_ID"].ToString() != String.Empty && Dt.Rows[i]["Order_No"].ToString() != String.Empty && Dt.Rows[i]["OrderColorID"].ToString() != String.Empty)
                    {
