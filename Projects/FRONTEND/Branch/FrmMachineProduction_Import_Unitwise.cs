...
namespace Accounts
{
    public partial class FrmMachineProduction_Import_Unitwise : Form
    {
        ...
        public FrmMachineProduction_Import_Unitwise(DateTime FDate, DateTime TDate, Int16 FShift, Int16 TShift, String FNeedle, String TNeedle, Int32 FYear, Int32 TYear, Int32 FWeek, Int32 TWeek, Int32 Unit)
        {
            InitializeComponent();
            MyBase.Clear(this);
        ...
        private void FrmMachineProduction_Import_Unitwise_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void FrmMachineProduction_Import_Unitwise_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    if (this.ActiveControl.Name == "TxtSample")
                    {
                        Sample_Selection1();
                    }
                }
        ...
        void Grid_Data()
        {
            try
            {
                ...
                //Commented On 06/Jun/2016 For Bom_Qty
                ...
                //Str = Str + " Left JOIn Socks_Machine_Production_Details S2 On S1.RowId = S2.Master_ID Left Join VFit_Sample_Needle_Master N1 On S1.Needle_ID = N1.RowID ";

                //if (UnitCode == 1)
                //{
                //    Str = Str + " Right Join Knitting_Mc_NO_UnitWise(1) M1 On S1.Machine_ID = M1.Machine_ID ";
                //}
                //else if (UnitCode == 2)
                //{
                //    Str = Str + " Right Join Knitting_Mc_NO_UnitWise(2) M1 On S1.Machine_ID = M1.Machine_ID ";
                //}
                //else
                //{
                //    Str = Str + " Left Join Knitting_Mc_No() M1 On S1.Machine_ID = M1.Machine_ID ";
                //}
                //Str = Str + " Left Join Socks_Bom() S3 On S2.Order_No = S3.Order_No And S2.OrderColorID = S3.OrderColorId Left Join Get_Knit_Production_OrderWise() P1 On S2.Order_No = P1.Order_No And S2.OrderColorID = P1.OrderColorID Where S1.Entry_Date = '" + String.Format("{0:dd-MMM-yyyy}", DtpFDate.Value) + "' And S1.Shift = " + TxtFShift.Text + " And S1.Needle_ID = " + TxtFNeedle.Tag + " And S1.Year = " + TxtFYear.Text + " And S1.Week = " + TxtFWeek.Text + " ";
                //Str = Str + " ORDER BY  LEFT(Machine, PATINDEX('%[0-9]%', Machine)-1), CONVERT(INT, SUBSTRING(Machine, PATINDEX('%[0-9]%', Machine), LEN(Machine)))";

                //Commented By Sakthi On 30-Dec-2016 For Speed Up
                //Str = " Select SNo, Needle_ID, Needle, Machine_ID, Machine, Order_No, OrderColorID, Sample, Size, Bom, Prod, Bal_Qty, Plan_Qty, Prod_Qty, Assign_Qty, Emplno, Operator, Planned_Seconds, Qty, Order_No2, 'N' T, Mode From Prod_Plan_Import('" + String.Format("{0:dd-MMM-yyyy}", DtpFDate.Value) + "', " + TxtFShift.Text + ", " + TxtFNeedle.Tag + ", " + TxtFYear.Text + ", " + TxtFWeek.Text + ", " + UnitCode + ")";
        ...
        void OrderNo_Selection()
        {
            try
            {
                ...
                if (UnitCode == 1)
                {
                    //Commented by Sakthi On 30/Dec/2016 For SpeedUp
                    //Str3 = " Select A.Order_No, A.Descr Description from Get_Sample_Production_Planning('" + TxtTNeedle.Text.ToString() + "')A Left Join JOb_Order_Unit_Details() B On A.Order_No = B.Order_No Where B.SupplierID = 71 ";

                    Str3 = " Select A.Order_No, Max(A.Descr) Descrpition From Socks_Order_Needle A Left Join JOb_Order_Unit_Details() B On A.Order_No = B.Order_No Where A.Needle = '" + TxtTNeedle.Text.ToString() + "' And B.SupplierID = 71 Group BY A.Order_No Order BY A.Order_No ";
                }
                else if (UnitCode == 2)
                {
                    //Commented by Sakthi On 30/Dec/2016 For SpeedUp
                    //Str3 = " Select A.Order_No, A.Descr Description from Get_Sample_Production_Planning('" + TxtTNeedle.Text.ToString() + "')A Left Join JOb_Order_Unit_Details() B On A.Order_No = B.Order_No Where B.SupplierID = 72 ";

                    Str3 = " Select A.Order_No, Max(A.Descr) Descrpition From Socks_Order_Needle A Left Join JOb_Order_Unit_Details() B On A.Order_No = B.Order_No Where A.Needle = '" + TxtTNeedle.Text.ToString() + "' And B.SupplierID = 72 Group BY A.Order_No Order BY A.Order_No ";
                }
                else if (UnitCode == 3)
                {
                    Str3 = " Select A.Order_No, Max(A.Descr) Descrpition From Socks_Order_Needle A Left Join JOb_Order_Unit_Details() B On A.Order_No = B.Order_No Where A.Needle = '" + TxtTNeedle.Text.ToString() + "' And B.SupplierID = 74 Group BY A.Order_No Order BY A.Order_No ";
                }
                else if (UnitCode == 4)
                {
                    Str3 = " Select A.Order_No, Max(A.Descr) Descrpition From Socks_Order_Needle A Left Join JOb_Order_Unit_Details() B On A.Order_No = B.Order_No Where A.Needle = '" + TxtTNeedle.Text.ToString() + "' And B.SupplierID = 75 Group BY A.Order_No Order BY A.Order_No ";
                }
                else
                {
        ...
        void Sample_Selection()
        {
            try
            {
                ...
                {
                    String Str3="";
                    //Comment On 04-Jun-2016 As per MD Instruction Qty Not Excessed BOM
                    ...
                    {
                        //Commented by Sakthi On 30/Dec/2016 For SpeedUp
                        //Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning('" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Where B.Order_No = '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "' And B.SupplierID = 71 ";

                        Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning_New('" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Left Join VFit_Sample_Master C On B.sample_No = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where B.Order_No = '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "' And B.SupplierID = 71 And D.Item != 'GUSSET' ";
                    }
                    else if (UnitCode == 2)
                    {
                        //Commented by Sakthi On 30/Dec/2016 For SpeedUp
                        //Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning('" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Where B.Order_No = '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "' And B.SupplierID = 72 ";

                        Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning_New('" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Left Join VFit_Sample_Master C On B.sample_No = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where B.Order_No = '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "' And B.SupplierID = 72 And D.Item != 'GUSSET' ";
                    }
                    else if (UnitCode == 3)
                    {
                        Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning_New('" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Left Join VFit_Sample_Master C On B.sample_No = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where B.Order_No = '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "' And B.SupplierID = 74 And D.Item != 'GUSSET' ";
                    }
                    else if (UnitCode == 4)
                    {
                        Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning_New('" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Left Join VFit_Sample_Master C On B.sample_No = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where B.Order_No = '" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "' And B.SupplierID = 75 And D.Item != 'GUSSET' ";
                    }
                    else
                    {
                        //Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning('" + Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() + "', " + TxtTNeedle.Tag.ToString() + ") A Left Join VFit_Sample_Master C On A.Sample = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where D.Item != 'GUSSET' ";
                    }
        ...
        void Sample_Selection1()
        {
            try
            {
                ...
                if (TxtOrder.Text.ToString() != String.Empty)
                {
                    ...
                    //Str3 = Str3 + " (S1.Bom_Qty - Isnull(G1.Production, 0))Bal_Qty, Isnull(G1.Production, 0) Prod ";
                    //Str3 = Str3 + " ,S1.OrderColorID, Datediff(SECOND, 0, cast('00:' + (Case When Cast(Getdate() As Date) >= G2.EffectFrom Then Isnull(G2.New_Cycle_Pair, S1.Cycle_Pair) Else S1.Cycle_Pair End) as time)) * (Case When (28800 / Datediff(SECOND, 0, cast('00:' + (Case When Cast(Getdate() As Date) >= G2.EffectFrom Then Isnull(G2.New_Cycle_Pair, S1.Cycle_Pair) Else S1.Cycle_Pair End) as time)) <= (S1.Bom_Qty - Isnull(G1.Production, 0))) Then (28800 / Datediff(SECOND, 0, cast('00:' + (Case When Cast(Getdate() As Date) >= G2.EffectFrom Then Isnull(G2.New_Cycle_Pair, S1.Cycle_Pair) Else S1.Cycle_Pair End) as time))) Else (S1.Bom_Qty - Isnull(G1.Production, 0)) End)Planned_Seconds ";
                    //Str3 = Str3 + " From Socks_Bom () S1 Left Join Get_Knit_Prod () G1 on S1.Order_No = G1.Order_No And S1.color = G1.Sample Left Join Get_Max_Cycle_Time() G2 On S1.OrderColorId = G2.OrderColorID Where S1.Order_No = '" + TxtOrder.Text.ToString() + "' And S1.Needle_ID = " + TxtTNeedle.Tag.ToString() + " ";

                    if (UnitCode == 1)
                    {
                        //Commented by Sakthi On 30/Dec/2016 For SpeedUp
                        //Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning('" + TxtOrder.Text + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Where B.Order_No = '" + TxtOrder.Text + "' And B.SupplierID = 71 ";

                        Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning_New('" + TxtOrder.Text + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Left Join VFit_Sample_Master C On B.sample_No = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where B.Order_No = '" + TxtOrder.Text + "' And B.SupplierID = 71 And D.Item != 'GUSSET' ";
                    }
                    else if (UnitCode == 2)
                    {
                        //Commented by Sakthi On 30/Dec/2016 For SpeedUp
                        //Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning('" + TxtOrder.Text + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Where B.Order_No = '" + TxtOrder.Text + "' And B.SupplierID = 72 ";

                        Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning_New('" + TxtOrder.Text + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Left Join VFit_Sample_Master C On B.sample_No = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where B.Order_No = '" + TxtOrder.Text + "' And B.SupplierID = 72 And D.Item != 'GUSSET' ";
                    }
                    else if (UnitCode == 3)
                    {
                        //Commented by Sakthi On 30/Dec/2016 For SpeedUp
                        ...
                        Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning_New('" + TxtOrder.Text + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Left Join VFit_Sample_Master C On B.sample_No = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where B.Order_No = '" + TxtOrder.Text + "' And B.SupplierID = 74 And D.Item != 'GUSSET' ";
                    }
                    else if (UnitCode == 4)
                    {
                        //Commented by Sakthi On 30/Dec/2016 For SpeedUp
                        ...
                        Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning_New('" + TxtOrder.Text + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join JOb_Order_Unit_Details_Samplewise()B On A.Sample = B.Sample_No Left Join VFit_Sample_Master C On B.sample_No = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where B.Order_No = '" + TxtOrder.Text + "' And B.SupplierID = 75 And D.Item != 'GUSSET' ";
                    }
                    else
                    {
                        //Str3 = " Select A.Sample, A.Size, A.BOM, A.Production, A.Qty, A.OrderColorID, A.Cycle_Seconds, A.Cycle_Time, A.Assign_Qty, A.Bal_Qty, A.Prod, A.Planned_Seconds From Get_Order_Sample_For_IMport_Produciton_planning('" + TxtOrder.Text + "', " + TxtTNeedle.Tag.ToString() + ")A Left Join VFit_Sample_Master C On A.Sample = C.Sample_No Left Join Item D On C.SampleItemID = D.ItemID Where D.Item != 'GUSSET' ";
                    }
                    Dr = Tool.Selection_Tool_WOMDI(this, 500, 180, SelectionTool_Class.ViewType.NormalView, "Select Sample", Str3, String.Empty, 100, 80, 80, 80);
                    if (Dr != null)
                    {
                        ...
                        //    if (Dt.Rows[i]["Sample"].ToString() != String.Empty)
                        //    {
                        //        if (Convert.ToString(TxtOrder.Text.ToString()) == Dt.Rows[i]["Order_No"].ToString() && Convert.ToString(TxtSample.Text.ToString()) == Dt.Rows[i]["Sample"].ToString() && Convert.ToString(TxtSize.Text.ToString())  == Dt.Rows[i]["Size"].ToString())
                        //        {
                        //            Grid["T", i].Value = "N";
                        //        }
                        //    }
        ...
        void Operator_Selection()
        {
            try
            {
                if (Grid["Order_No", Grid.CurrentCell.RowIndex].Value.ToString() != String.Empty && Grid["Sample", Grid.CurrentCell.RowIndex].Value.ToString() != String.Empty)
                {
                    ...
                    Str3 = " Select Name, Tno, Emplno From Vaahini_ERP_Gainup.Dbo.Employeemas E1 Inner Join Vaahini_ERP_Gainup.Dbo.Depttype D1 on E1.Deptcode = D1.DeptCode and E1.COMPCODE = D1.compcode Where E1.tno not like '%Z'";

                    if (UnitCode == 1)
                    {
                        Str3 = Str3 + " And E1.compcode IN (2) and D1.deptCode = 82 And E1.Unit_Code = 1 ";
                    }
                    else if (UnitCode == 2)
                    {
                        Str3 = Str3 + " And E1.compcode IN (2) and D1.deptCode = 82 And E1.Unit_Code = 2 ";
                    }
                    else if (UnitCode == 3)
                    {
                        Str3 = Str3 + " And E1.compcode IN (8) and D1.deptCode = 209 And E1.Unit_Code = 3 ";
                    }
                    else if (UnitCode == 4)
                    {
                        Str3 = Str3 + " And E1.compcode IN (8) and D1.deptCode = 209 And E1.Unit_Code = 4 ";
                    }
