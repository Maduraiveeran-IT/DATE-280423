...
namespace Accounts
{
    public partial class FrmProjectGeneralPOApproval : Form, Entry
    {
        ...
        public FrmProjectGeneralPOApproval()
        {
            InitializeComponent();
        }
        ...
        public void Entry_Save()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < Grid_Item.Rows.Count - 1; i++)
                {
                    ...
                    Grid_Item["RATE", i].Value = String.Format("{0:0.00}", Convert.ToDouble(Grid_Item["GRS_RATE", i].Value) + Convert.ToDouble(String.Format("{0:0.00}", ((Convert.ToDouble(Grid_Item["GRS_RATE", i].Value) * Convert.ToDouble(Grid_Item["TAX_PER", i].Value)) / 100))) + 0 + 0);
                ...
                for (int i = 0; i < Grid_Item.Rows.Count - 1; i++)
                {
                    MyBase.SQLCmd.CommandText = "Insert into VSocks_Sample_Req_Details (Master_ID, Slno, Product_Id, ItemID, SizeID, ColorID, Req_Qty, Rate, Slno1, Dyeing_Mode, Dyeing_Item_ID,  Grs_Rate, Tax_Per, Remarks, Dye_Rate, Tax_Per_Dye, Dye_Pur_Rate, Pur_rate) Values (" + SMasCode + ",  " + (i + 1) + ", 0 , " + Grid_Item["Item_ID", i].Value + ", " + Grid_Item["Size_ID", i].Value + " , " + Grid_Item["Color_ID", i].Value + ",  " + Grid_Item["PO_Qty", i].Value + ",  " + Grid_Item["Rate", i].Value + ", " + (i + 1) + ", 'N', 0 , " + Grid_Item["Grs_Rate", i].Value + ", " + Grid_Item["Tax_Per", i].Value + ", 'AUTO SAVE FOR GENERAL PO ', 0, 0, 0,0)";
                    MyBase.SQLCmd.ExecuteNonQuery();
                }
                ...
                {
                    MyBase.SQLCmd.CommandText = "update Socks_General_PO_Master Set Buyer_Code = " + TxtBuyer.Tag.ToString() + ", Supplier_Code = " + TxtSupplier.Tag.ToString() + ", Required_Date = '" + String.Format("{0:dd-MMM-yyyy}", DtpReqDate.Value) + "' Where RowID = " + Code;
                    MyBase.SQLCmd.ExecuteNonQuery();
                    ...
                    for (int i = 0; i < Grid_Item.Rows.Count - 1; i++)
                    {
                        if (Convert.ToDouble(Grid_Item["PO_Qty", i].Value.ToString()) > 0)
                        {
                            if (MyParent._New)
                            {
                                MyBase.SQLCmd.CommandText = "Insert into Socks_General_PO_Details (Master_ID, Slno, Order_ID, Item_ID, Color_ID, Size_ID, Order_Qty, Grs_Rate, Tax_Per, Rate, Amount) Values (" + PMasCode + ", " + (i + 1) + ", " + SMasCode + ", " + Grid_Item["Item_ID", i].Value.ToString() + ", " + Grid_Item["Color_ID", i].Value.ToString() + ", " + Grid_Item["Size_ID", i].Value.ToString() + ", " + Grid_Item["PO_Qty", i].Value.ToString() + ", " + Grid_Item["Grs_Rate", i].Value.ToString() + ", " + Grid_Item["Tax_Per", i].Value.ToString() + ", " + Grid_Item["Rate", i].Value.ToString() + ", " + (Convert.ToDouble(Grid_Item["PO_Qty", i].Value.ToString()) * Convert.ToDouble(Grid_Item["Rate", i].Value.ToString())) + ")";
                            }
                            else
                            {
                                MyBase.SQLCmd.CommandText = "Insert into Socks_General_PO_Details (Master_ID, Slno, Order_ID, Item_ID, Color_ID, Size_ID, Order_Qty, Grs_Rate, Tax_Per, Rate, Amount) Values (" + Code + ", " + (i + 1) + ", " + SMasCode + ", " + Grid_Item["Item_ID", i].Value.ToString() + ", " + Grid_Item["Color_ID", i].Value.ToString() + ", " + Grid_Item["Size_ID", i].Value.ToString() + ", " + Grid_Item["PO_Qty", i].Value.ToString() + ", " + Grid_Item["Grs_Rate", i].Value.ToString() + ", " + Grid_Item["Tax_Per", i].Value.ToString() + ", " + Grid_Item["Rate", i].Value.ToString() + ", " + (Convert.ToDouble(Grid_Item["PO_Qty", i].Value.ToString()) * Convert.ToDouble(Grid_Item["Rate", i].Value.ToString())) + ")";
                            }
                            MyBase.SQLCmd.ExecuteNonQuery();
                            Slno++;
                        }
        ...
        public void Entry_Print()
        {
            try
            {
                ...
                if (Dt1.Rows[0][0].ToString().Contains("GUP-POS"))
                {
                    MyParent.FormulaFill(ref ObjRpt, "Heading", "GENERAL YARN PURCHASE ORDER");
                }
                else
                {
                ...
                {
                    MyParent.CReport(ref ObjRpt, "General Yarn Purchase Order..!");
                }
                else if (Res == DialogResult.No)
                {
                    ...
                    MyParent.CReport_Normal_PDF(ref ObjRpt, "General Yarn Purchase Order..!", "C:\\Vaahrep\\GainupPO.Pdf", false);
        ...
        private void FrmProjectGeneralPOApproval_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void FrmProjectGeneralPOApproval_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                else if (e.KeyCode == Keys.Down)
                {
                    ...
                    if (this.ActiveControl.Name == "TxtPONO")
                    {
                        ...
                        MyBase.Clear(this);
                        lblMail.Text = "";
                        Dr = Tool.Selection_Tool_Resize(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Select PO Details - Approve", "Select S1.PONo, L1.Ledger_Name Supplier, S1.PoDate, S3.Order_No, i1.item, c1.color, s4.size, s2.Order_Qty, s2.Rate, S1.RowID, S1.PO_Method, S1.Supplier_Code, S1.Buyer_Code, L2.Ledger_Name Buyer, S1.Required_Date, L1.LEdger_Email From Fitsocks.Dbo.Socks_General_PO_Master S1 Inner join Fitsocks.Dbo.Socks_General_PO_Details S2 ON S1.RowID = s2.Master_ID Inner join Fitsocks.Dbo.VSocks_Sample_Req_Master S3 on S2.Order_ID = S3.RowID inner join Fitsocks.Dbo.item I1 on S2.Item_id = I1.itemid inner join Fitsocks.Dbo.color C1 on s2.Color_id = c1.colorid inner join Fitsocks.Dbo.size S4 on s2.Size_ID = S4.sizeid left Join Fitsocks.Dbo.Supplier_All_Fn() L1 On L1.LEdgeR_Code = S1.Supplier_Code  Left Join Fitsocks.Dbo.Buyer_All_Fn() L2 On L2.LEdgeR_code = S1.Buyer_Code Where S1.Approval_Flag = 'F' Order by 1 desc  ", String.Empty, 120, 250, 100, 120, 120, 120, 100, 100, 150);
                        if (Dr != null)
                        {
                            Fill_Data(Dr);
                        }
        ...
        void Load_Item()
        {
            try
            {
                //if (MyParent._New)
                //{
                //    Grid_Item.DataSource = MyBase.Load_Data("Select 0 as Sl, Size + ' ' + Item + ' ' + Color + ' @ ' + Cast(Rate as Varchar (15)) Description, Item_ID, Item ITEM, Color_ID, Color COLOR, Size_ID, size SIZE, Sum(BOM_Qty) BOM_Qty, Sum(Ordered_Qty) Ordered_Qty, Sum(PO_Qty) Bal_Qty, Sum(PO_Qty) PO_Qty, GRS_RATE, TAX_PER, RATE ARATE, RATE, SUM(Amount) AMOUNT From SocksPO_General_Pending_Items (5275) where 1 = 2 Group by Item_ID, Item, Color_ID, Color, Size_ID, size, Rate, Grs_Rate, Tax_Per", ref Dt_Item);
                //}
                //else
                //{
                    // Also Change in Load_Dt_Item_OCN for Edit
                Grid_Item.DataSource = MyBase.Load_Data("Select 0 as Sl, S2.Size + ' ' + I1.Item + ' ' + C1.Color + ' @ ' + Cast(S3.Rate as Varchar (15)) Description, S1.Item_ID, I1.Item ITEM, S1.Color_ID, C1.Color COLOR, S1.Size_ID, S2.size SIZE, (S1.BOM_Qty) BOM_Qty, ((S1.Ordered_Qty)) Ordered_Qty, ((S1.PO_Qty) + (S3.Order_Qty)) Bal_Qty, (S3.Order_Qty) PO_Qty, S3.GRS_RATE, S3.TAX_PER, S3.RATE ARATE, S3.RATE, (S3.Amount) AMOUNT From Fitsocks.Dbo.SocksPO_General_Pending_Items_All() S1 Inner Join Fitsocks.Dbo.Socks_General_PO_Details S3 on S1.Order_ID = S3.Order_ID and S1.Item_ID = S3.Item_id and S1.Color_ID = S3.Color_id and S1.Size_ID = S3.Size_ID Inner Join Fitsocks.Dbo.item I1 on S1.Item_id = I1.itemid Inner Join Fitsocks.Dbo.color C1 on s1.Color_id = c1.colorid Inner Join Fitsocks.Dbo.size S2 on s1.Size_ID = S2.sizeid Where Master_ID = " + Code + " ", ref Dt_Item);
                //}
                
                MyBase.Grid_Designing(ref Grid_Item, ref Dt_Item, "Item_ID", "Color_ID", "Size_ID", "Description", "ARATE", "BOM_QTY", "ORDERED_QTY", "BAL_QTY");
                MyBase.ReadOnly_Grid_Without(ref Grid_Item, "Item", "Color", "Size", "PO_Qty", "GRS_RATE", "TAX_PER");
                MyBase.Grid_Colouring(ref Grid_Item, Control_Modules.Grid_Design_Mode.Column_Wise);
                MyBase.Grid_Width(ref Grid_Item, 30, 150, 150, 150, 100, 100, 80, 100, 110);
        ...
        private void FrmProjectGeneralPOApproval_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if (this.ActiveControl is TextBox)
        ...
        void Txt_Item_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (Grid_Item.CurrentCell.ColumnIndex == Grid_Item.Columns["GRS_RATE"].Index && Txt_Item.Text.ToString() != String.Empty)
                {
                    ...
                    {
                        Grid_Item["RATE", Grid_Item.CurrentCell.RowIndex].Value = String.Format("{0:0.00}", Convert.ToDouble(Grid_Item["GRS_RATE", Grid_Item.CurrentCell.RowIndex].Value) + Convert.ToDouble(String.Format("{0:0.00}", ((Convert.ToDouble(Grid_Item["GRS_RATE", Grid_Item.CurrentCell.RowIndex].Value) * Convert.ToDouble(Grid_Item["TAX_PER", Grid_Item.CurrentCell.RowIndex].Value)) / 100))) + 0 + 0);
        ...
        void Txt_Item_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Down)
                {
                    ...
                    {
                        if (TxtBuyer.Text.ToString() == String.Empty || TxtSupplier.Text.ToString() == String.Empty)
                        {
                        ...
                        e.Handled = true;
                        Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Item", "Select Item, ItemID From ITem Where ITem Not like '%ZZZ%' and Len(LTrim(Rtrim(Item))) >=1 ORder by 1", String.Empty, 250);
                        if (Dr != null)                        
                        {
                            MyBase.Row_Number(ref Grid_Item);
                            ...
                            Grid_Item["ARATE", Grid_Item.CurrentCell.RowIndex].Value = String.Format("{0:0.000}", 0);
                            Grid_Item["Rate", Grid_Item.CurrentCell.RowIndex].Value = String.Format("{0:0.000}", 0);
                            Grid_Item["Grs_Rate", Grid_Item.CurrentCell.RowIndex].Value = String.Format("{0:0.000}", 0);
                    ...
                    {
                        e.Handled = true;
                        Dr = Tool.Selection_Tool(this, 30, 70, SelectionTool_Class.ViewType.NormalView, "Size", "Select Size, Item_Type, SizeID From Size Where Size Not like '%ZZZ%' and Len(LTrim(Rtrim(Size))) >=1 ORder by 1", String.Empty, 200,120);
                        if (Dr != null)
                        {                            
        ...
        private void ButApp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MyBase.Run("Update Fitsocks.Dbo.Socks_General_PO_Master Set Approval_Flag = 'T' , Approval_System = Host_Name(), Approval_Time = Getdate() Where RowID = " + Code + " ");
                
               // MyBase.Run(MyParent.EntryLog("SOCKS GENERAL PO APPROVAL", "APPROVE", Code.ToString()));
